...
export function useBombPartyLobby() {
	...
	const isAuthenticating = useBombPartyStore(
		(state) => state.connection.isAuthenticating
	);
	const connectionState = useBombPartyStore(
	...
	const handleLobbyCreate = useCallback(
		(
			...
			if (!playerId || isAuthenticating) {
				...
				if (isAuthenticating) {
					useBombPartyStore
						.getState()
						.setLastError(
							"Authentification en cours, veuillez patienter quelques secondes..."
						);
				} else {
					useBombPartyStore
						.getState()
			...
			if (connectionState !== "connected") {
				logger.warn(
					"Impossible de creer le lobby: connexion non etablie",
					{ connectionState }
				);
				useBombPartyStore
					.getState()
					.setLastError("Connexion non etablie. Veuillez reessayer.");
				return;
			}

			if (!meta.name || meta.name.trim().length === 0) {
				logger.error(
					"Impossible de creer le lobby: nom invalide",
					undefined,
					{ name: meta.name }
				);
				useBombPartyStore
					.getState()
					.setLastError("Le nom du lobby ne peut pas être vide");
				return;
			}

			if (meta.maxPlayers < 2 || meta.maxPlayers > 12) {
				logger.error(
					"Impossible de creer le lobby: nombre de joueurs invalide",
					undefined,
					{ maxPlayers: meta.maxPlayers }
				);
				useBombPartyStore
					.getState()
					.setLastError(
						"Le nombre de joueurs doit être entre 2 et 12"
					);
				return;
			}
			...
			try {
				...
				logger.error("Erreur lors de la creation du lobby", err, {
					playerId,
					meta,
					error: err,
				});
				useBombPartyStore
					.getState()
					.setLastError("Erreur lors de la creation du lobby");
			}
		},
	...
	const handleLobbyJoin = useCallback(
		(roomId: string, password: string | undefined, client?: any) => {
			if (!playerId || isAuthenticating) {
				logger.warn(
					"Impossible de rejoindre le lobby: non authentifie",
					{ playerId, isAuthenticating, roomId }
				);
				useBombPartyStore
					.getState()
					.setLastError(
						"Vous devez être connecte pour rejoindre un lobby"
					);
				return;
			}
			...
			if (connectionState !== "connected") {
				logger.warn(
					"Impossible de rejoindre le lobby: connexion non etablie",
			...
			if (!roomId || roomId.trim().length === 0) {
				logger.error(
					"Impossible de rejoindre le lobby: roomId invalide",
					undefined,
					{ roomId }
				);
				useBombPartyStore
					.getState()
					.setLastError("ID de lobby invalide");
				return;
			}

			logger.info("Rejoindre le lobby via bombPartyService", {
				roomId,
				playerId,
			...
			try {
				...
				logger.error("Erreur lors de la connexion au lobby", err, {
					playerId,
					roomId,
				});
				useBombPartyStore
					.getState()
					.setLastError("Erreur lors de la connexion au lobby");
			}
		},
		[playerId, isAuthenticating, connectionState]
	);
	...
	const handleLeaveLobby = useCallback(
		(roomId: string | null, client: BombPartyClient) => {
			if (roomId && client) {
				try {
					client.sendMessage({
					...
					logger.error(
						"Erreur lors de la deconnexion du lobby",
						err,
						{ roomId }
					);
				}
	...
	const handleStartGame = useCallback(
		(
			...
			if (!roomId) {
				logger.error(
					"Impossible de demarrer: pas de roomId",
					undefined,
					{ isHost, playersCount: lobbyPlayers.length }
				);
				useBombPartyStore.getState().setLastError("Room ID manquant");
				return;
			}
			if (!isHost) {
				logger.warn("Impossible de demarrer: pas hôte", {
					roomId,
					isHost,
				});
				useBombPartyStore
					.getState()
					.setLastError("Seul l'hôte peut demarrer la partie");
				return;
			}
			if (lobbyPlayers.length < 2) {
				logger.warn("Impossible de demarrer: pas assez de joueurs", {
					roomId,
					playersCount: lobbyPlayers.length,
				});
				useBombPartyStore
					.getState()
					.setLastError("Il faut au moins 2 joueurs pour demarrer");
				return;
			}
			...
			if (store.connection.state !== "connected") {
				logger.warn("Impossible de demarrer: connexion non etablie", {
					connectionState: store.connection.state,
					roomId,
				});
				store.setLastError(
					"Connexion non etablie. Veuillez reessayer."
				);
				return;
			}
			...
			logger.info("Envoi du demarrage au serveur via bombPartyService", {
				roomId,
				playersCount: lobbyPlayers.length,
			});
			...
			try {
				...
				logger.error("Erreur lors du demarrage du jeu", err, {
					roomId,
				});
				useBombPartyStore
					.getState()
					.setLastError("Erreur lors du demarrage du jeu");
				startingGameRef.current = false;
			}
		},
		[]
