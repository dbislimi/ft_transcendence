...
export function useBombPartyGame(user: any) {
	...
	useEffect(() => {
		if (
			...
			const isNewTurn =
				previousTurnStartedAt !== undefined &&
				previousTurnStartedAt !== newTurnStartedAt;
			const needsInitialization =
				turnStartTime === 0 || turnStartTime !== newTurnStartedAt;

			if (needsInitialization) {
				console.log(
					"[useBombPartyGame] 🎯 Mise à jour de turnStartTime",
					{
				...
				const turnDuration =
					...
					gameState.baseTurnSeconds * 1000 ||
					12000;
				if (turnDuration > 0) {
					...
					console.log(
						"[useBombPartyGame] Timer demarre, isActive:",
						timer.isTimerActive()
					);
				}
			}
		...
		if (gameMode === "local" && gameState.phase === "TURN_ACTIVE") {
			if (turnStartTime === 0) {
				const now = Date.now();
				console.log(
					"[useBombPartyGame] Initialisation de turnStartTime en mode local",
					{
						time: now,
						currentPlayer:
							gameState.players[gameState.currentPlayerIndex]
		...
		if (gameState.phase !== "TURN_ACTIVE") {
			...
			if (timer.isTimerActive()) {
				console.log(
					"[useBombPartyGame] Arrêt du timer - phase changee",
					{
			...
			if (
				...
				console.log(
					"[useBombPartyGame] Phase changee, conservation de turnStartTime pour traçabilite",
					{
						oldPhase: "TURN_ACTIVE",
						newPhase: gameState.phase,
						turnStartTime,
	...
	useEffect(() => {
		if (gameState.phase === "GAME_OVER" && gameStartTime && playerId) {
			...
			const redirectTimer = setTimeout(() => {
				console.log(
					"[useBombPartyGame] Partie terminee, redirection vers l'ecran d'accueil"
				);
				alert(
					t(
						"bombParty.gameOver.redirectToMenu",
						"Partie terminee. Retour à l'ecran d'accueil..."
					)
				);
				setGamePhase("RULES");
			}, 5000);
	...
	const handleWordSubmit = useCallback(
		async (
			...
			if (turnStartTime === 0) {
				console.error(
					"[useBombPartyGame] ⚠️ ERREUR: turnStartTime est 0 lors de la soumission!",
					{
						gameMode,
						...
						serverTurnStartedAt: gameState.turnStartedAt,
						message:
							"Le turnStartTime aurait dû être initialise au debut du tour",
					}
				);
				if (gameState.turnStartedAt) {
					console.warn(
						"[useBombPartyGame] 🔄 Utilisation du fallback avec gameState.turnStartedAt"
					);
					setTurnStartTime(gameState.turnStartedAt);
				} else {
					console.error(
						"[useBombPartyGame] ❌ ERREUR CRITIQUE: Aucun temps serveur disponible, calcul de msTaken sera incorrect",
						{
							gameMode,
							phase: gameState.phase,
				...
				console.warn(
					"[useBombPartyGame] ⚠️ Desynchronisation detectee, resynchronisation avec le serveur",
					{
						turnStartTime,
						serverTurnStartedAt: gameState.turnStartedAt,
			...
			const responseTime =
				turnStartTime > 0 ? Date.now() - turnStartTime : 0;
			bombPartyStatsService.recordTrigramAttempt(
			...
			if (gameMode === "local") {
				...
				try {
					...
					console.error(
						"[useBombPartyGame] Error validating word with backend, falling back to local:",
						error
					);
				}
				...
				if (result.ok) {
					(async () => {
						await engine.resolveTurn(true, false);
						...
						if (!engine.isGameOver()) {
							setTimeout(() => {
								...
								const newTurnStart =
									updatedState.turnStartedAt || Date.now();
								const newTurnDuration =
									updatedState.turnDurationMs || 15000;
					...
					if (result.consumedDoubleChance) {
						...
						(async () => {
							await engine.resolveTurn(false, false);
							const newState = engine.getState();
							setGameState(newState);
				...
				if (!roomId || !playerId) {
					console.error(
						"[useBombPartyGame] Missing roomId or playerId in multiplayer mode",
						{
							roomId,
							playerId,
						}
				...
				if (turnStartTime > 0) {
					...
					if (
						...
						console.warn(
							"[useBombPartyGame] ⚠️ turnStartTime desynchronise du serveur, utilisation du temps serveur",
							{
								turnStartTime,
								serverTurnStartedAt: gameState.turnStartedAt,
								diff: Math.abs(
									turnStartTime - gameState.turnStartedAt
								),
							}
						);
					...
					msTaken = clientNow - gameState.turnStartedAt;
					console.warn(
						"[useBombPartyGame] ⚠️ turnStartTime est 0, utilisation de gameState.turnStartedAt",
						{
					...
					console.error(
						"[useBombPartyGame] ❌ ERREUR CRITIQUE: Impossible de calculer msTaken",
						{
							turnStartTime,
							serverTurnStartedAt: gameState.turnStartedAt,
							phase: gameState.phase,
				...
				if (msTaken < 0) {
					console.error(
						"[useBombPartyGame] ❌ msTaken negatif detecte, correction à 0",
						{
							msTaken,
							turnStartTime,
							serverTurnStartedAt: gameState.turnStartedAt,
				...
				if (msTaken > gameState.turnDurationMs + 2000) {
					console.warn(
						"[useBombPartyGame] ⚠️ msTaken suspect (trop grand), limitation",
						{
							msTaken,
					...
					msTaken = Math.min(
						msTaken,
						gameState.turnDurationMs + 1000
					);
				}
				console.log(
					"[useBombPartyGame] ✅ Submitting word in multiplayer",
					{
						word,
						msTaken,
						turnStartTime,
				...
				try {
					...
					console.error(
						"[useBombPartyGame] Error submitting word",
						error
					);
					setWordJustSubmitted(false);
					setTurnInProgress(false);
	...
	const handleActivateBonus = useCallback(
		(
			...
			if (gameMode === "local") {
				...
				if (!roomId || !playerId) {
					console.error(
						"Pas de roomId ou playerId pour activer le bonus"
					);
					return false;
				}
	...
	const isCurrentPlayerTurn = useCallback(
		(gameMode: "local" | "multiplayer", playerId: string | null) => {
			if (gameMode === "local") {
