...
export function useBombPartyWebSocket(
	user: any,
	options?: UseBombPartyWebSocketOptions
) {
	...
	const setGameStartTime = useBombPartyStore(
		(state) => state.setGameStartTime
	);
	const setWordJustSubmitted = useBombPartyStore(
		(state) => state.setWordJustSubmitted
	);
	const setTurnInProgress = useBombPartyStore(
		(state) => state.setTurnInProgress
	);
	const setTurnStartTime = useBombPartyStore(
		(state) => state.setTurnStartTime
	);
	const setTimerGracePeriod = useBombPartyStore(
		(state) => state.setTimerGracePeriod
	);
	const setTimerFlash = useBombPartyStore((state) => state.setTimerFlash);
	const setBonusNotification = useBombPartyStore(
		(state) => state.setBonusNotification
	);
	...
	const setLobbyMaxPlayers = useBombPartyStore(
		(state) => state.setLobbyMaxPlayers
	);
	...
	useEffect(() => {
		if (playerId !== storeConnection.playerId)
			setPlayerId(storeConnection.playerId);
		if (roomId !== storeConnection.roomId)
			setRoomId(storeConnection.roomId);
		...
		const amIHost = !!(
			first &&
			storeConnection.playerId &&
			first === storeConnection.playerId
		);
		if (amIHost !== storeConnection.isHost) setIsHost(amIHost);
	}, [
		storeConnection.playerId,
		storeConnection.roomId,
		storeConnection.lobbyPlayers,
		storeConnection.lobbyMaxPlayers,
		storeConnection.isHost,
		playerId,
		roomId,
		setLobbyPlayers,
		setLobbyMaxPlayers,
	...
	useEffect(() => {
		const checkPrimaryConnection = () => {
			...
			const hasPrimaryService = connectionsInfo.allConnections.some(
				(c) =>
					c.type === "bombPartyService" &&
					c.isActive &&
					c.id === connectionsInfo.primaryConnection
			);
			if (hasPrimaryService) {
				logger.debug(
					"BombPartyService dejà actif, ne pas creer de connexion concurrente avec BombPartyClient"
				);
				return false;
			}
			return true;
		...
		const connect = () => {
			if (!checkPrimaryConnection()) {
				logger.debug(
					"Connexion BombPartyClient annulee - BombPartyService est dejà primaire"
				);
				return;
			}
			...
			if (!user || user.id === -1) {
				logger.debug(
					"User is guest, skipping BombPartyClient connection"
				);
				return;
			}
		...
		const handleConnected = () => {
			...
			if (user?.id) {
				...
				playerName =
					(storedName && storedName.trim()) || `Guest_${guestId}`;
			}

			logger.debug("Authentification avec le nom", {
				playerName,
				userId: user?.id,
			...
			const authTimeout = setTimeout(() => {
				...
				const isInGame =
					currentStore.gamePhase === "GAME" &&
					currentStore.gameState?.phase === "TURN_ACTIVE";
				...
				if (hasPlayerId) {
					logger.info(
						"Authentication already succeeded, ignoring timeout"
					);
					return;
				}

				const connectionsInfo = wsCoordinator.getConnectionsInfo();
				const isServicePrimary =
					connectionsInfo.primaryConnection &&
				...
				if (isInGame) {
					logger.warn(
						"Game in progress, not resetting auth - will retry later"
					);
					setTimeout(() => {
						if (playerIdRef.current === null) {
							...
							if (isServicePrimary) {
								bombPartyService.authenticateWithName(
									retryPlayerName
								);
							} else {
								logger.warn(
									"BombPartyService not primary, skipping authentication retry"
								);
							}
						}
					}, 2000);
				...
				if (!isConnected) {
					logger.warn(
						"Authentication timeout but connection not established, skipping re-auth",
						{
				...
				if (isServicePrimary) {
					...
					logger.warn(
						"BombPartyService not primary, skipping authentication"
					);
				}
		...
		const handleLobbyJoined = (payload: any) => {
			...
			if (isMyJoin) {
				...
				logger.debug(
					"Message concernant un autre joueur, conservation de mon statut isHost"
				);
			}
		...
		const handleGameEnd = (payload: any) => {
			logger.info(
				"[useBombPartyWebSocket] Jeu termine - nettoyage du timer"
			);
		...
		const handleLobbyListUpdate = (payload: any) => {
			logger.debug("Liste des lobbies mise à jour", {
		...
		const handleGameStart = (payload: any) => {
			...
			if (payload.roomId === curRoom) {
				logger.info("Demarrage du jeu pour la room", {
					roomId: payload.roomId,
				});
				setGamePhase("GAME");
				setCountdown(0);
		...
		const handleGameState = (payload: any) => {
			...
			if (!payload?.roomId || payload.roomId !== curRoom) {
				console.warn(
					"[useBombPartyWebSocket] Ignorer l'etat du jeu pour une autre room",
					{
						currentRoomId: curRoom,
						payloadRoomId: payload?.roomId,
					}
				);
				logger.debug("Ignorer l'etat du jeu pour une autre room", {
					currentRoomId: curRoom,
					payloadRoomId: payload?.roomId,
				});
				return;
			...
			if (payload.gameState.phase === "GAME_OVER") {
				logger.info(
					"[useBombPartyWebSocket] GAME_OVER detecte dans handleGameState"
				);
				if (timerRef.current && timerRef.current.isTimerActive()) {
					timerRef.current.stop();
				}
			...
			if (payload.gameState.phase === "TURN_ACTIVE") {
				...
				setTurnInProgress(false);
				const turnDuration =
					payload.gameState.turnDurationMs ||
					payload.gameState.baseTurnSeconds * 1000;
				...
				if (serverTurnStart && turnDuration > 0) {
					...
					if (timerRef.current) {
						console.log(
							"[useBombPartyWebSocket] Demarrage du timer",
							{
								serverTurnStart,
								turnDuration,
						...
						timerRef.current.startTurn(
							serverTurnStart,
							turnDuration,
							Date.now()
						);
						console.log(
							"[useBombPartyWebSocket] Timer demarre, isActive:",
							timerRef.current.isTimerActive()
						);
					} else {
						console.warn(
							"[useBombPartyWebSocket] timerRef.current est null !"
						);
					}
					...
					logger.warn(
						"Donnees de tour invalides - turnStartTime non initialise",
						{
							serverTurnStart,
							turnDuration,
							phase: payload.gameState.phase,
				...
				payload.gameState.phase !== "TURN_ACTIVE" &&
				timerRef.current &&
				timerRef.current.isTimerActive()
			) {
				logger.info(
					"[useBombPartyWebSocket] Phase non-TURN_ACTIVE detectee - arret du timer",
					{
						phase: payload.gameState.phase,
					}
				);
		...
		const handleBonusApplied = (payload: any) => {
			...
			if (
				...
				const player = currentState?.players?.find(
					(p: any) => p.id === payload.playerId
				);
				const playerName = player?.name || "Un joueur";

				setBonusNotification({
					bonusKey: payload.bonusKey as BonusKey,
					playerName,
				});

				if (
					payload.bonusKey === "plus5sec" &&
					payload.meta?.extendMs &&
					timerRef.current
				) {
					...
					if (
						...
						currentState?.phase === "TURN_ACTIVE"
					) {
				...
				if (
					payload.bonusKey === "vitesseEclair" &&
					payload.meta?.targetId
				) {
		...
		const unsubscribeCreated = client.on(
			"bp:lobby:created",
			handleLobbyCreated
		);
		const unsubscribeJoined = client.on(
			"bp:lobby:joined",
			handleLobbyJoined
		);
		const unsubscribePlayerJoined = client.on(
			"bp:lobby:player_joined",
			handlePlayerJoined
		);
		const unsubscribeRoomState = client.on(
			"bp:room:state",
			handleRoomState
		);
		const unsubscribePlayerLeft = client.on(
			"bp:lobby:player_left",
			handlePlayerLeft
		);
		const unsubscribeLobbyList = client.on(
			"bp:lobby:list" as any,
			handleLobbyListUpdate
		);
		const unsubscribeLobbyListUpdated = client.on(
			"bp:lobby:list_updated" as any,
			handleLobbyListUpdate
		);
		const unsubscribeGameCountdown = client.on(
			"bp:game:countdown",
			handleGameCountdown
		);
		const unsubscribeGameStart = client.on(
			"bp:game:start",
			handleGameStart
		);
		const unsubscribeGameState = client.on(
			"bp:game:state",
			handleGameState
		);
		const unsubscribeWordResult = client.on(
			"bp:game:word_result",
			handleWordResult
		);
		...
		const unsubscribeBonusApplied = client.on(
			"bp:bonus:applied" as any,
			handleBonusApplied
		);
