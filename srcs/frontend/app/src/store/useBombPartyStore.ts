...
import type { Player, BonusKey } from '@shared/bombparty/types';
...
export type ConnectionStateType = 'disconnected' | 'connecting' | 'connected' | 'reconnecting' | 'server_unreachable';
...
export interface BombPartyStore {
  ...
  setOptimisticLifeLoss: (lifeLoss: { playerId: string; timestamp: number } | null) => void;
...
export const useBombPartyStore = create<BombPartyStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    receiveServerState: (gameState) => {
      try {
        if (!gameState) {
          logger.warn('receiveServerState called with undefined gameState', {
      ...
      if (optimisticLoss && currentState) {
        const serverPlayer = gameState.players?.find((p: any) => p.id === optimisticLoss.playerId);
        const currentPlayer = currentState.players?.find((p: any) => p.id === optimisticLoss.playerId);
    ...
    applyGameStateDelta: (delta, sequenceNumber) => {
      ...
      if (delta.turnStartedAt !== undefined) {
        ...
        if (delta.phase === 'TURN_ACTIVE' || updatedState.phase === 'TURN_ACTIVE') {
          const currentTurnStartTime = get().turnStartTime;
          const newTurnStartedAt = delta.turnStartedAt;
          
          if (newTurnStartedAt && newTurnStartedAt !== currentTurnStartTime) {
            logger.info('🎯 applyGameStateDelta - Mise à jour de turnStartTime depuis delta', {
              currentTurnStartTime,
              newTurnStartedAt,
              isNewTurn: currentTurnStartTime > 0 && currentTurnStartTime !== newTurnStartedAt,
              sequenceNumber,
              deltaKeys: Object.keys(delta)
            });
      ...
      if (delta.players !== undefined) {
        if (Array.isArray(delta.players)) {
          if (delta.players.length > 0 && delta.players[0].index !== undefined) {
            const playersCopy = [...updatedState.players];
            for (const playerDelta of delta.players) {
              if (playerDelta.index !== undefined && playerDelta.player) {
                playersCopy[playerDelta.index] = playerDelta.player;
              }
            }
    ...
    submitWord: (word, msTaken) => {
      import('../services/bombPartyService').then(({ bombPartyService }) => {
        try {
          ...
          logger.error('Erreur lors de la soumission du mot', err);
          get().setLastError('Erreur lors de la soumission du mot');
        }
      });
    },
    ...
    activateBonus: (bonusKey) => {
      import('../services/bombPartyService').then(({ bombPartyService }) => {
        try {
          ...
          logger.error('Erreur lors de l\'activation du bonus', err);
          get().setLastError('Erreur lors de l\'activation du bonus');
        }
      });
    },
    ...
    joinRoom: (roomId, password) => {
      import('../services/bombPartyService').then(({ bombPartyService }) => {
        try {
          ...
          logger.error('Erreur lors de la connexion à la room', err);
          get().setLastError('Erreur lors de la connexion à la room');
        }
      });
    },
    ...
    setLobbyPlayers: (lobbyPlayers) => set((prev) => {
      if (prev.connection.lobbyPlayers === lobbyPlayers || 
          (prev.connection.lobbyPlayers.length === lobbyPlayers.length &&
           prev.connection.lobbyPlayers.every((p, i) => p.id === lobbyPlayers[i]?.id && p.name === lobbyPlayers[i]?.name))) {
        return prev;
      }
    ...
    setLobbyMaxPlayers: (lobbyMaxPlayers) => set((prev) => {
      if (prev.connection.lobbyMaxPlayers === lobbyMaxPlayers) return prev;
      return { connection: { ...prev.connection, lobbyMaxPlayers } };
    }),
    
    setIsAuthenticating: (isAuthenticating) => set((prev) => {
      if (prev.connection.isAuthenticating === isAuthenticating) return prev;
      return { connection: { ...prev.connection, isAuthenticating } };
    }),
    
    setReconnectAttempts: (reconnectAttempts) => set((prev) => {
      if (prev.connection.reconnectAttempts === reconnectAttempts) return prev;
      return { connection: { ...prev.connection, reconnectAttempts } };
    }),
    ...
    setWordJustSubmitted: (wordJustSubmitted) => set((prev) => {
      if (prev.ui.wordJustSubmitted === wordJustSubmitted) return prev;
    ...
    setLobbies: (lobbies) => set((prev) => {
      if (prev.lobbies === lobbies || 
    ...
    requestLobbyList: () => {
      import('../services/bombPartyService').then(({ bombPartyService }) => {
        try {
          ...
          logger.warn('Erreur lors de la requête de la liste des lobbies', { error: err });
        }
      });
    },
    
    setPlayerNameModalOpen: (playerNameModalOpen) => set((prev) => {
      if (prev.playerNameModalOpen === playerNameModalOpen) return prev;
      return { playerNameModalOpen };
    }),
    
    setGameMode: (gameMode) => set((prev) => {
      if (prev.gameMode === gameMode) return prev;
      return { gameMode };
    }),
    setMultiplayerType: (multiplayerType) => set((prev) => {
      if (prev.multiplayerType === multiplayerType) return prev;
      return { multiplayerType };
    }),
    setCountdown: (countdown) => set((prev) => {
      if (prev.countdown === countdown) return prev;
      return { countdown };
    }),
    setGameStartTime: (gameStartTime) => set((prev) => {
      if (prev.gameStartTime === gameStartTime) return prev;
      return { gameStartTime };
    }),
    setBonusNotification: (bonusNotification) => set((prev) => {
      if (prev.bonusNotification === bonusNotification ||
          (prev.bonusNotification?.bonusKey === bonusNotification?.bonusKey &&
           prev.bonusNotification?.playerName === bonusNotification?.playerName)) {
        return prev;
      }
    ...
    setTurnStartTime: (turnStartTime) => {
      ...
      logger.info('🎯 setTurnStartTime appelé', {
        ...
        diff: prev.turnStartTime > 0 ? turnStartTime - prev.turnStartTime : 'initial',
        isNewTurn: prev.turnStartTime > 0 && prev.turnStartTime !== turnStartTime,
    ...
    setOptimisticLifeLoss: (lifeLoss: { playerId: string; timestamp: number } | null) => set({ optimisticLifeLoss: lifeLoss }),
    ...
    getRemainingTime: () => {
      ...
      if (turnStartTime > 0 && Math.abs(turnStartTime - gameState.turnStartedAt) < 1000) {
        ...
        logger.warn('getRemainingTime - Aucune source de temps valide', {
          turnStartTime,
      ...
      if (remaining < 0 || remaining > gameState.turnDurationMs + 2000) {
        logger.warn('getRemainingTime - Calcul suspect', {
          ...
          phase: gameState.phase,
          source,
          timeDiff: turnStartTime > 0 ? Math.abs(turnStartTime - gameState.turnStartedAt) : 'N/A'
        });
      }
    ...
    canSubmitWord: () => {
      ...
      if (gameState.phase !== 'TURN_ACTIVE') {
        logger.debug('canSubmitWord - Phase not TURN_ACTIVE', { phase: gameState.phase });
        return false;
      }
    ...
    revertOptimisticWordSubmit: (word: string) => {
      ...
      const updatedUsedWords = gameState.usedWords.filter(w => w !== normalizedWord);
      ...
      logger.debug('Optimistic update reverted', { word: normalizedWord });
    }
  }))
);
