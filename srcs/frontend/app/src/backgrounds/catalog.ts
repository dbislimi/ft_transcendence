import {
	fortniteBackgrounds,
	type FortniteBackgroundItem,
} from "./fortnite-catalog";

const backgroundFiles = import.meta.glob(
	"/img/background/*.{svg,webp,png,jpg,jpeg}",
	{
		eager: true,
		query: "?url",
		import: "default",
...
function buildCatalog(): BackgroundItem[] {
	...
	for (const [path, url] of Object.entries(backgroundFiles)) {
		...
		if (id === "42" || generatedId === "42background") continue;
	...
	const theme42Path = Object.keys(backgroundFiles).find((path) =>
		path.includes("42background")
	);
	if (theme42Path) {
...
const fortniteBackgroundItems: BackgroundItem[] = fortniteBackgrounds.map(
	(fb) => ({
		id: fb.id,
		name: fb.name,
		url: fb.url,
...
export const allBackgrounds: BackgroundItem[] = [
	...backgroundCatalog,
	...fortniteBackgroundItems,
];
...
export function getBackgroundsByType(
	type: "image" | "svg" | "default"
): BackgroundItem[] {
