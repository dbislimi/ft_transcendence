...
export default function ChatWidget() {
	...
	const blockUser = (userId: number, userName: string) => {
		chatWsRef.current?.send(
			JSON.stringify({ type: "block", userId, name: userName })
		);
	...
	const unblockUser = (userId: number, userName: string) => {
		chatWsRef.current?.send(
			JSON.stringify({ type: "unblock", userId, name: userName })
		);
	};
	...
	const handleInvite = (friendId: number) => {
		if (
			pongWsRef.current &&
			pongWsRef.current.readyState === WebSocket.OPEN
		) {
