...
export function BombPartyStatsSummary({ userStats, globalStats, isAuthenticated }: BombPartyStatsSummaryProps) {
    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-2xl font-bold text-white mb-4 flex items-center gap-2">
          ...
          {t('bombParty.stats.global.title') || 'Statistiques Globales'}
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="bg-gradient-to-br from-blue-600/20 to-cyan-600/20 p-6 rounded-lg border border-blue-500/30">
            <div className="text-4xl font-bold text-blue-300 mb-2">{globalStats.totalPlayers}</div>
            <div className="text-gray-300">{t('bombParty.stats.global.totalPlayers') || 'Joueurs'}</div>
          </div>
          <div className="bg-gradient-to-br from-purple-600/20 to-pink-600/20 p-6 rounded-lg border border-purple-500/30">
            <div className="text-4xl font-bold text-purple-300 mb-2">{globalStats.totalMatches}</div>
            <div className="text-gray-300">{t('bombParty.stats.global.totalMatches') || 'Parties jouées'}</div>
          </div>
          <div className="bg-gradient-to-br from-green-600/20 to-emerald-600/20 p-6 rounded-lg border border-green-500/30">
            <div className="text-4xl font-bold text-green-300 mb-2">{globalStats.totalValidWords}</div>
            <div className="text-gray-300">{t('bombParty.stats.global.totalWords') || 'Mots validés'}</div>
          </div>
        </div>
      </div>
      ...
      {isAuthenticated && userStats && (
        <div>
          <h2 className="text-2xl font-bold text-white mb-4 flex items-center gap-2">
            ...
            {t('bombParty.stats.personal.title') || 'Mes Statistiques'}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-800 p-6 rounded-lg">
              <h3 className="text-lg font-semibold text-cyan-400 mb-4">{t('bombParty.stats.overview.otherStats')}</h3>
              <div className="space-y-3">
                <div className="flex justify-between">
