...
export default function BombPartyStatsContainer() {
	...
	>(user?.id && hasToken ? "overview" : "ranking");
	...
	const loadUserData = async () => {
		...
		try {
			if (user?.id) {
				logger.debug("Loading complete data for user", {
					userId: user.id,
				});

				try {
					const [statsResponse, historyResponse, rankingResponse] =
						await Promise.all([
					...
					logger.warn(
						"Error loading authenticated stats, loading ranking only",
						{ error: authError }
					);
					const rankingResponse =
						await bombPartyStatsService.getGlobalRanking();
					setGlobalRanking(rankingResponse.data || []);
				...
				logger.debug("Global ranking loaded", {
					count: rankingResponse.data?.length || 0,
				});
	...
	if (error) {
		return (
			<div className="flex items-center justify-center py-12">
				<div className="text-center">
					<svg
						className="w-24 h-24 mx-auto mb-4 text-red-400"
						fill="none"
						stroke="currentColor"
						viewBox="0 0 24 24"
					>
