	...
	const toggle2FA = async () => {
		try {
			...
			if (res.ok) {
				...
				notify({
					variant: "success",
					message: data.twoFAEnabled
						? t("settings.account.twoFAActivated")
						: t("settings.account.twoFADeactivated"),
				});
			} else {
				notify({
					variant: "error",
					message: t("settings.account.twoFAError"),
				});
			}
		} catch (error) {
			notify({
				variant: "error",
				message: t("settings.account.connectionError"),
			});
		}
	};
	...
	const handleSave = async (e: React.FormEvent) => {
		...
		try {
			if (newPassword && newPassword !== confirmPassword) {
				notify({
					variant: "error",
					message: t("settings.account.passwordMismatch"),
				});
				setIsLoading(false);
				return;
			}
			...
			if (res.ok) {
				notify({
					variant: "success",
					message: t("settings.account.profileUpdated"),
				});
				await refreshUser();
				setIsEditing(false);
				setCurrentPassword("");
				...
				notify({
					variant: "error",
					message: data.error || t("settings.account.updateError"),
				});
			}
		} catch (error) {
			notify({
									...
									<div>
										...
										<div className="flex items-center justify-between p-3 bg-slate-900/50 border border-slate-600 rounded-lg">
											...
											>
												{is2FAEnabled
													? t(
															"settings.account.disableTwoFA"
													  )
													: t(
															"settings.account.enableTwoFA"
													  )}
											</button>
										</div>
									</div>
