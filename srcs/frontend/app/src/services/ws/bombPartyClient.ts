...
export class BombPartyClient {
	...
	private hasLoggedReconnectionStopped: boolean = false;

	constructor(options: BombPartyClientOptions = {}) {
		this.mock = options.mock ?? false;
		this.connectionId = `bpc_${Math.random()
			.toString(36)
			.substring(2, 10)}`;
	...
	private cleanup(): void {
		if (this.isCleaningUp) {
			logger.debug(
				`[BombPartyClient] Cleanup already in progress, skipping [${this.connectionId}]`
			);
			return;
		}
		...
		logger.debug(
			`[BombPartyClient] Starting cleanup [${this.connectionId}]`
		);
		...
		try {
			...
			for (const cleanup of this.cleanupCallbacks) {
				try {
					...
					logger.error(
						`[BombPartyClient] Error in cleanup callback:`,
						err
					);
				}
			}
			...
			if (this.ws) {
				try {
					...
					logger.warn("error cleaning up websocket handlers", {
						connectionId: this.connectionId,
						error: err,
					});
				}
			...
			logger.debug(
				`[BombPartyClient] Cleanup completed [${this.connectionId}]`
			);
		} finally {
	...
	public connect(): void {
		if (this.shouldStopReconnecting) {
			logger.error(
				`[BombPartyClient] Reconnection permanently stopped - max attempts reached [${this.connectionId}]`,
				{
			...
			logger.warn(
				`[BombPartyClient] Call reset() method to allow reconnection attempts again [${this.connectionId}]`
			);
			return;
		}
		...
		if (this.isDisconnected) {
			...
			logger.debug(
				`[BombPartyClient] Resetting isDisconnected flag [${this.connectionId}]`
			);
		}

		if (this.connectionLock) {
			logger.debug(
				`[BombPartyClient] Connection lock active, ignoring connect request [${this.connectionId}]`
			);
			return;
		}
		...
		if (
			...
			logger.debug(
				`[BombPartyClient] Connection already in progress or active, ignored [${this.connectionId}]`
			);
			return;
		}
		...
		if (!this.registeredWithCoordinator) {
			...
			logger.warn(
				`[BombPartyClient] Not priority for now (attempt ${this.nonPriorityAttempts}/${this.maxNonPriorityAttempts}) [${this.connectionId}]`,
				{
			...
			if (this.nonPriorityAttempts >= this.maxNonPriorityAttempts) {
				if (hasActiveService) {
					logger.error(
						`[BombPartyClient] Max non-priority attempts reached (${this.maxNonPriorityAttempts}), stopping reconnection [${this.connectionId}] - BombPartyService is active`
					);
					this.shouldStopReconnecting = true;
					this.stopReconnectingAndCleanQueue();
					this._emit("error", {
						error: "Not priority, connection abandoned after max attempts - BombPartyService is active",
					});
					return;
				} else {
					logger.warn(
						`[BombPartyClient] Max non-priority attempts reached but no BombPartyService active, will retry with longer delay [${this.connectionId}]`
					);
					const delay = this.calculateReconnectDelay(
						this.maxNonPriorityAttempts
					);
			...
			const delay = this.calculateReconnectDelay(
				this.nonPriorityAttempts - 1
			);
			logger.debug(
				`[BombPartyClient] Scheduling non-priority retry in ${Math.round(
					delay
				)}ms [${this.connectionId}]`
			);
		...
		logger.debug(
			`[BombPartyClient] WebSocket connection attempt... [${this.connectionId}]`
		);

		try {
			const wsUrl = this.getWebSocketUrl();
			logger.debug(
				`[BombPartyClient] Connecting to: ${wsUrl.replace(
					/token=[^&]+/,
					"token=***"
				)}`
			);
			...
			this.connectionTimeout = setTimeout(() => {
				if (this.ws && this.ws.readyState === WebSocket.CONNECTING) {
					logger.warn(
						`[BombPartyClient] Connection timeout [${this.connectionId}]`
					);
					this.ws.close();
			...
			this.ws.onopen = () => {
				...
				if (this.shouldStopReconnecting) {
					logger.debug(
						`[BombPartyClient] Connection successful, resetting shouldStopReconnecting flag [${this.connectionId}]`
					);
				...
				logger.debug(
					`[BombPartyClient] Connexion etablie [${this.connectionId}]`
				);
			...
			this.ws.onmessage = (event) => {
				if (!wsCoordinator.isPrimaryConnection(this.connectionId)) {
					logger.debug(
						`[BombPartyClient] Not priority, message ignored [${this.connectionId}]`
					);
					return;
				}
				...
				try {
					...
					if (!isBPServerMessage(rawData)) {
						logger.warn(
							`[BombPartyClient] Invalid message format received:`,
							{
								connectionId: this.connectionId,
					...
					logger.error(
						"[BombPartyClient] Error parsing message:",
						err
					);
				}
			};
			...
			this.ws.onclose = (event) => {
				...
				logger.debug(
					`[BombPartyClient] Connection closed [${
						this.connectionId
				...
				if (this.shouldStopReconnecting) {
					logger.error(
						`[BombPartyClient] Connection closed but reconnection permanently stopped [${this.connectionId}]`,
						{
							code: event.code,
					...
					if (this.reconnectTimer) {
						...
						this.reconnectTimer = null;
						logger.debug(
							`[BombPartyClient] Cleared pending reconnect timer [${this.connectionId}]`
						);
					}
					logger.warn(
						`[BombPartyClient] Call reset() to allow new connection attempts [${this.connectionId}]`
					);
					return;
				}
				...
				if (event.code === 1000) {
					logger.debug(
						`[BombPartyClient] Normal closure (code 1000), not reconnecting [${this.connectionId}]`
					);
					this.reconnectAttempts = 0;
					return;
				}
				...
				if (this.reconnectAttempts < this.maxReconnectAttempts) {
					...
					this.reconnectAttempts++;
					logger.debug(
						`[BombPartyClient] Scheduling reconnect ${
							this.reconnectAttempts
						}/${this.maxReconnectAttempts} in ${Math.round(
							delay
						)}ms [${this.connectionId}]`
					);
					...
					logger.error(
						`[BombPartyClient] Max reconnect attempts reached (${this.maxReconnectAttempts}), stopping permanently [${this.connectionId}]`
					);
					this.shouldStopReconnecting = true;
					this.stopReconnectingAndCleanQueue();
					...
					if (this.reconnectTimer) {
						...
						this.reconnectTimer = null;
						logger.debug(
							`[BombPartyClient] Cleared all reconnect timers [${this.connectionId}]`
						);
					}
			...
			this.ws.onerror = (error) => {
				...
				logger.error(
					`[BombPartyClient] WebSocket error [${this.connectionId}]:`,
					errorInfo
				);
			...
			this.isConnecting = false;
			logger.error(
				`[BombPartyClient] Exception lors de la connexion [${this.connectionId}]:`,
				err
			);

			if (err instanceof Error && err.message.includes("token")) {
				logger.warn(
					`[BombPartyClient] Authentication error, not retrying [${this.connectionId}]`
				);
				return;
			}
	...
	private handleConnectionFailure(): void {
		...
		if (this.shouldStopReconnecting) {
			logger.error(
				`[BombPartyClient] Connection failure but reconnection permanently stopped [${this.connectionId}]`,
				{
		...
		if (this.reconnectAttempts < this.maxReconnectAttempts) {
			...
			this.reconnectAttempts++;
			logger.debug(
				`[BombPartyClient] Connection failed, scheduling retry ${
					this.reconnectAttempts
			...
			this._emit("error", {
				error: "Max reconnect attempts reached after connection failure",
			});
			logger.warn(
				`[BombPartyClient] Call reset() to allow new connection attempts [${this.connectionId}]`
			);
	...
	public disconnect(): void {
		if (this.isDisconnecting || this.isDisconnected) {
			logger.debug(
				`[BombPartyClient] Disconnect already in progress or done [${this.connectionId}]`
			);
			return;
		}

		logger.debug(
			`[BombPartyClient] Disconnect requested [${this.connectionId}]`
		);
		...
		if (this.ws) {
			try {
				logger.debug(
					`[BombPartyClient] WebSocket disconnection [${this.connectionId}]`
				);
	...
	public reset(): void {
		logger.debug(
			`[BombPartyClient] Reset requested [${this.connectionId}]`
		);
		...
		logger.debug(
			`[BombPartyClient] Reset completed, client can reconnect [${this.connectionId}]`
		);
	}
	...
	private stopReconnectingAndCleanQueue(): void {
		...
		if (queueLength > 0) {
			if (!this.hasLoggedReconnectionStopped) {
				logger.warn(
					`[BombPartyClient] Reconnection stopped - clearing message queue (${queueLength} messages) [${this.connectionId}]`,
					{
						message:
							"Call reset() to re-enable connection attempts",
					}
				);
	...
	private addToQueue(event: string, payload: unknown): void {
		...
		if (this.messageQueue.length >= this.messageQueueMaxSize) {
			...
			logger.warn(
				`[BombPartyClient] Message queue full, removing least priority message [${this.connectionId}]`
			);
		}
	...
	private cleanExpiredMessages(): void {
		...
		if (this.messageQueue.length < initialLength) {
			logger.debug(
				`[BombPartyClient] Removed ${
					initialLength - this.messageQueue.length
				} expired messages [${this.connectionId}]`
			);
		}
	}
	...
	private sendMessageDirectly(event: string, payload: unknown): boolean {
		...
		try {
			...
			if (event !== "bp:ping" && event !== "bp:pong") {
				logger.debug(
					`[BombPartyClient] Message sent to WebSocket [${this.connectionId}]:`,
					{
						event,
			...
			logger.error(
				`[BombPartyClient] Error sending message [${this.connectionId}]:`,
				err
			);
			return false;
		}
	...
	emit(event: BombPartyEvent, payload: unknown): void {
		...
		if (event !== "bp:ping" && event !== "bp:pong") {
			logger.debug(
				`[BombPartyClient] emit called [${this.connectionId}]:`,
				{
					event,
		...
		if (this.ws && this.ws.readyState === WebSocket.OPEN) {
			if (this.sendMessageDirectly(event, payload)) {
				if (event !== "bp:ping" && event !== "bp:pong") {
					logger.debug("message sent directly", {
						connectionId: this.connectionId,
						event,
					});
				}
				...
				logger.warn("failed to send message directly", {
					connectionId: this.connectionId,
					event,
				});
			}
		...
		if (event !== "bp:ping" && event !== "bp:pong") {
			...
			logger.debug(
				`[BombPartyClient] WebSocket not ready, queuing message [${this.connectionId}]:`,
				{
					event,
					wsState: this.ws?.readyState,
			...
			if (
				...
				logger.debug(
					`[BombPartyClient] Reconnection attempt to send message [${this.connectionId}]`
				);
				this.connect();
			}
		}
	...
	authenticate(playerName: string): void {
		...
		if (!wsCoordinator.isPrimaryConnection(this.connectionId)) {
			logger.debug(
				`[BombPartyClient] Not primary connection, skipping authentication [${this.connectionId}]`
			);
			return;
		}
		...
		if (this.authenticated && this.currentPlayerName === playerName) {
			logger.debug(
				`[BombPartyClient] Already authenticated as [${playerName}], ignored [${this.connectionId}]`
			);
			return;
		}

		logger.debug(
			`[BombPartyClient] Send authentication for [${playerName}] [${this.connectionId}]`
		);
		this.currentPlayerName = playerName;

		if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
			logger.debug(
				`[BombPartyClient] WebSocket not connected during authentication, queueing and connecting... [${this.connectionId}]`
			);
	...
	createLobby(
		...
		logger.debug(
			`[BombPartyClient] createLobby called [${this.connectionId}]`,
			{
				name,
				isPrivate,
		...
		this.emit("bp:lobby:create", {
			name,
			isPrivate,
			password,
			maxPlayers,
		} as LobbyCreatePayload);
	}
	...
	activateBonus(roomId: string, bonusKey: BonusKey): void {
		this.emit("bp:bonus:activate", {
			roomId,
			playerId: "current",
			bonusKey,
		} as BonusActivatePayload);
	}
	...
	private _emit(event: string, payload: unknown): void {
		...
		if (event === "connected") {
			...
			logger.debug(
				`[BombPartyClient] Authentication successful [${this.connectionId}]`
			);
	...
	private _processPendingMessages(): void {
		...
		logger.debug(
			`[BombPartyClient] Processing ${this.messageQueue.length} pending messages [${this.connectionId}]`
		);
		...
		for (const message of messagesToSend) {
			if (Date.now() > message.expiresAt) {
				logger.debug(
					`[BombPartyClient] Skipping expired message: ${message.event} [${this.connectionId}]`
				);
				continue;
			}
			...
			if (this.ws && this.ws.readyState === WebSocket.OPEN) {
				if (this.sendMessageDirectly(message.event, message.payload)) {
					logger.debug(
						`[BombPartyClient] Queued message sent: ${message.event} [${this.connectionId}]`
					);
				} else {
