...
export class BombPartyService {
	...
	constructor(roomId: string = "default") {
		...
		if (typeof import.meta.hot !== "undefined") {
			logger.debug("Hot reload support detected (Vite)", {
				connectionId: this.connectionId,
			});
			...
			import.meta.hot.on("vite:beforeUpdate", () => {
				...
				if (isConnected) {
					this.hotReloadDetected = true;
					logger.info(
						"Hot reload: beforeUpdate - preservation de la connexion ouverte",
						{
							connectionId: this.connectionId,
							wsState: wsStateName,
							isConnected: true,
					...
					logger.info(
						"Hot reload: beforeUpdate - connexion fermee, pas de preservation",
						{
							connectionId: this.connectionId,
							wsState: wsStateName,
							isConnected: false,
			...
			import.meta.hot.on("vite:afterUpdate", () => {
				...
				if (this.hotReloadDetected && isConnected) {
					logger.info(
						"Hot reload: afterUpdate - connexion preservee avec succes",
						{
							connectionId: this.connectionId,
							wsState: wsStateName,
							isConnected: true,
					...
					logger.info(
						"Hot reload: afterUpdate - connexion fermee, reconnexion necessaire",
						{
							connectionId: this.connectionId,
							wsState: wsStateName,
							isConnected: false,
					...
					const storedName = sessionStorage.getItem(
						"bombparty_player_name"
					);
					const token = sessionStorage.getItem("token");
					const playerName =
						storedName || (token ? "AuthenticatedUser" : null);

					setTimeout(() => {
						logger.info(
							"Hot reload: tentative de reconnexion apres hot reload",
							{
								connectionId: this.connectionId,
								hadRoomId,
								hadPlayerId,
						...
						if (playerName && hadRoomId) {
							const checkConnection = setInterval(() => {
								if (this.ws?.readyState === WebSocket.OPEN) {
									...
									if (playerName) {
										logger.info(
											"Hot reload: reauthentification apres reconnexion",
											{
												connectionId: this.connectionId,
									...
									if (hadRoomId) {
										setTimeout(() => {
											logger.info(
												"Hot reload: rejoindre la room apres reconnexion",
												{
													connectionId:
														this.connectionId,
													roomId: hadRoomId,
				...
				setTimeout(() => {
					this.hotReloadDetected = false;
					logger.debug("Hot reload flag reset", {
						connectionId: this.connectionId,
					});
				}, 1000);
			});
	...
	public init(): void {
		console.log("🎮 [BombParty] init() called", {
			isInitialized: this.isInitialized,
		...
		if (this.isInitialized) {
			...
			if (this.ws?.readyState === WebSocket.OPEN) {
				logger.debug("Connection already open, skipping reconnection", {
					connectionId: this.connectionId,
				});
				return;
			}
			...
			if (isHotReload && this.hotReloadDetected) {
				logger.debug(
					"Hot reload detected, preserving connection state",
					{ connectionId: this.connectionId }
				);
				return;
			}
		...
		logger.debug("Manual initialization", {
			connectionId: this.connectionId,
		});
	...
	private connect(): void {
		...
		if (!token) {
			...
			logger.info("No token found - connecting as guest", {
				connectionId: this.connectionId,
			});
		} else {
			console.log("🎮 [BombParty] Has token - connecting with auth");
			logger.debug("Connecting with authentication token", {
				connectionId: this.connectionId,
			});
		}
		...
		if (this.ws?.readyState === WebSocket.CONNECTING) {
			logger.warn(
				"Connection in progress (CONNECTING state), ignoring duplicate request",
				{
					connectionId: this.connectionId,
					wsState: this.ws.readyState,
				}
		...
		if (
			...
			logger.warn(
				"WebSocket already connected or connecting, ignoring duplicate request",
				{
					connectionId: this.connectionId,
		...
		if (this.ws) {
			logger.debug(
				"Cleaning up old WebSocket before creating new connection",
				{
					connectionId: this.connectionId,
		...
		if (!this.registeredWithCoordinator) {
			logger.debug(
				"Not authorized by coordinator, connection cancelled",
				{ connectionId: this.connectionId }
			);
		...
		try {
			...
			console.log(
				"🎮 [BombParty] WebSocket created, setting up handlers..."
			);
	...
	private setupEventHandlers(): void {
		if (!this.ws) {
			logger.warn("setupEventHandlers called but ws is null", {
				connectionId: this.connectionId,
			});
			return;
		}
		...
		if (this.ws.onopen !== null) {
			logger.warn(
				"Handlers already attached, cleaning before re-attaching",
				{ connectionId: this.connectionId }
			);
		...
		this.ws.onopen = () => {
			...
			if (hasAuthUser && storedName && storedName.startsWith("Guest_")) {
				...
				logger.info(
					"Cleared guest name for authenticated user at connection",
					{
						connectionId: this.connectionId,
						oldName: storedName,
					}
			...
			logger.debug("Auth decision", {
				hasAuthUser,
				storedName: storedName ? "PRESENT" : "MISSING",
				pending: this.pendingPlayerName ? "PRESENT" : "MISSING",
			});
			...
			if (hasAuthUser) {
				...
				try {
					...
					realName =
						...
						payload.name ||
						`User_${payload.id || Date.now().toString().slice(-6)}`;
					...
					logger.warn(
						"Failed to parse JWT, backend will provide name",
						{
							connectionId: this.connectionId,
							error,
						}
				...
				if (storedName && storedName.startsWith("Guest_")) {
					sessionStorage.removeItem("bombparty_player_name");
					logger.info(
						"Nettoyage du nom de guest stocké pour utilisateur authentifié"
					);
				}
			...
			if (store.connection.roomId) {
				setTimeout(() => {
					if (
						this.ws?.readyState === WebSocket.OPEN &&
						store.connection.playerId &&
						store.connection.roomId
					) {
						logger.info(
							"🔄 Demande automatique de l'etat de la room apres reconnexion",
							{
								connectionId: this.connectionId,
		...
		this.ws.onmessage = (event) => {
			const isPrimary = wsCoordinator.isPrimaryConnection(
				this.connectionId
			);
			...
			try {
				...
				if (
					message.event !== "bp:ping" &&
					message.event !== "bp:pong"
				) {
	...
	private isDuplicateMessage(message: BombPartyMessage): boolean {
		...
		const messageKey = `${message.event}_${JSON.stringify(
			message.payload || {}
		)}`;
		const now = Date.now();
	...
	private handleMessage(message: BombPartyMessage): void {
		...
		if (message.t === "error") {
			...
			if (
				(error.code === "STATE_ERROR" ||
					error.code === "ROOM_NOT_FOUND") &&
				/room not found/i.test(error.msg || "")
			) {
				const hadRoom = !!store.connection.roomId;
				const gameInProgress =
					store.gameState && store.gameState.phase === "TURN_ACTIVE";
				if (hadRoom && !gameInProgress) {
					logger.info(
						"Room not found on server (no game in progress), resetting local room state"
					);
					...
					logger.warn(
						"Room not found but game in progress, preserving game state"
					);
				}
			}
			...
			if (typeof window !== "undefined") {
				...
				window.dispatchEvent(
					new CustomEvent("bp:lobby:error", {
						detail: {
							error: error.msg,
							code: errorCode,
						},
		...
		switch (message.event) {
			...
			case "bp:auth:success":
				...
				if (message.payload.playerName) {
					const currentStored = sessionStorage.getItem(
						"bombparty_player_name"
					);
					...
					if (!message.payload.playerName.startsWith("Guest_")) {
						...
						if (
							currentStored &&
							currentStored.startsWith("Guest_")
						) {
							logger.info(
								"Replaced guest name with real username from backend",
								{
									connectionId: this.connectionId,
			...
			case "bp:lobby:created":
				...
				if (this.ws?.readyState !== WebSocket.OPEN) {
					...
					store.setLastError(
						"Connexion perdue pendant la creation du lobby"
					);
					return;
				}
			...
			case "bp:lobby:joined":
				...
				if (isReconnect || store.gameState) {
					logger.info(
						"🔄 Reconnexion detectee - demande de l'etat de la room",
						{
							connectionId: this.connectionId,
							roomId: message.payload.roomId,
							isReconnect,
			...
			case "bp:lobby:player_joined":
				...
				if (store.connection.roomId === message.payload.roomId) {
					...
					if (
						...
						message.payload.playerId !== store.connection.playerId
					) {
						window.dispatchEvent(
							new CustomEvent("bp:lobby:player_joined", {
								detail: {
									playerId: message.payload.playerId,
									playerName: message.payload.playerName,
									roomId: message.payload.roomId,
			...
			case "bp:lobby:player_left":
				...
				if (store.connection.roomId === message.payload.roomId) {
					...
					if (
						...
						window.dispatchEvent(
							new CustomEvent("bp:lobby:player_left", {
								detail: {
									playerId: message.payload.playerId,
									playerName: message.payload.playerName,
									roomId: message.payload.roomId,
			...
			case "bp:lobby:left":
				if (message.payload.playerId === store.connection.playerId) {
					logger.info("Nous avons quitte le lobby", {
						roomId: message.payload.roomId,
					});
					store.setRoomId(null);
					store.setIsHost(false);
					...
					if (store.connection.roomId === message.payload.roomId) {
						...
						if (
							...
							const first = message.payload.players[0]?.id;
							store.setIsHost(
								first === store.connection.playerId
							);
						}
					}
				}
			...
			case "bp:lobby:host_disconnected":
				...
				if (store.connection.roomId === message.payload.roomId) {
					...
					if (typeof window !== "undefined") {
						window.dispatchEvent(
							new CustomEvent("bp:lobby:host_disconnected", {
								detail: {
									roomId: message.payload.roomId,
									hostName: message.payload.hostName,
									reason: message.payload.reason,
			...
			case "bp:game:countdown":
				logger.debug("Game countdown received", {
					payload: message.payload,
				});
				...
				if (message.payload?.roomId === store.connection.roomId) {
					...
					logger.info("Countdown demarre", {
						startTime,
						duration,
						roomId: message.payload.roomId,
						countdown: Math.ceil(
							(startTime + duration - Date.now()) / 1000
						),
					});
				}
				break;
			...
			case "bp:game:start":
				logger.debug("Game start received", {
					payload: message.payload,
				});
				...
				if (message.payload?.roomId === store.connection.roomId) {
					...
					logger.info("Jeu demarre", {
						roomId: message.payload.roomId,
					});
					if (typeof window !== "undefined") {
						window.dispatchEvent(
							new CustomEvent("bp:game:start", {
								detail: {
									roomId: message.payload.roomId,
								},
							})
			...
			case "bp:game:input:received":
				logger.debug("Game input received confirmation", {
					payload: message.payload,
				});
				if (typeof window !== "undefined") {
					window.dispatchEvent(
						new CustomEvent("bp:game:input:received", {
			...
			case "bp:game:state":
				try {
					...
					if (
						...
						if (
							newGameState.phase === "TURN_ACTIVE" &&
							newTurnStartedAt
						) {
							const isNewTurn =
								newTurnStartedAt !== currentTurnStartTime;
							const needsUpdate =
								isNewTurn || currentTurnStartTime === 0;
							...
							if (needsUpdate || timeDiff > 100) {
								logger.info(
									"🎯 bp:game:state (full) - Synchronisation du timer",
									{
										connectionId: this.connectionId,
										...
										timeOffset:
											Date.now() - newTurnStartedAt,
										turnDurationMs:
											newGameState.turnDurationMs,
										sequenceNumber:
											gameStatePayload.sequenceNumber,
									}
								);
								store.setTurnStartTime(newTurnStartedAt);
							} else {
								logger.debug(
									"bp:game:state (full) - Timer dejà synchronise",
									{
										connectionId: this.connectionId,
							...
							logger.debug(
								"bp:game:state (full) - Phase changee, turnStartTime conserve",
								{
									connectionId: this.connectionId,
						...
						if (gameStatePayload.isReconnect) {
							logger.info(
								"etat complet du jeu reçu apres reconnexion",
								{
									connectionId: this.connectionId,
						...
						gameStatePayload.delta &&
						!gameStatePayload.full
					) {
						...
						if (delta.phase === "TURN_ACTIVE" && newTurnStartedAt) {
							...
							if (
								...
								logger.info(
									"🎯 bp:game:state (delta) - Synchronisation du timer",
									{
										connectionId: this.connectionId,
										currentTurnStartTime,
										newTurnStartedAt,
								...
								logger.debug(
									"bp:game:state (delta) - Timer dejà synchronise",
									{
										connectionId: this.connectionId,
										turnStartTime: currentTurnStartTime,
										serverTurnStartedAt: newTurnStartedAt,
						...
						if (gameStatePayload.isReconnect) {
							logger.info(
								"etat delta du jeu reçu apres reconnexion",
								{
									connectionId: this.connectionId,
									roomId: gameStatePayload.roomId,
									sequenceNumber:
			...
			case "bp:game:end":
				...
				if (currentState && endPayload.winner) {
					...
					logger.debug("Winner added to gameState", {
						winnerId: endPayload.winner.id,
					});
				}
				if (typeof window !== "undefined") {
					window.dispatchEvent(
						new CustomEvent("bp:game:end", {
							detail: {
			...
			case "bp:game:rejoin_prompt":
				logger.debug("Game rejoin prompt received", {
					payload: message.payload,
				});
				const rejoinPayload = message.payload;
				if (typeof window !== "undefined") {
					window.dispatchEvent(
						new CustomEvent("bp:game:rejoin_prompt", {
							detail: {
			...
			case "bp:turn:started":
				logger.debug("Turn started received", {
					payload: message.payload,
				});
				...
				if (
					...
					if (payload.roomId === store.connection.roomId) {
						...
						const isNewTurn =
							currentTurnStartTime !== newTurnStartedAt;

						logger.info(
							"🎯 bp:turn:started - Mise à jour de turnStartTime",
							{
						...
						logger.info(
							"Tour demarre - turnStartTime mis à jour avec le temps serveur"
						);
					}
				}
				break;
			...
			case "bp:pong":
				...
				const latency = message.payload?.ts
					? pongReceivedAt - message.payload.ts
					: "unknown";
				const timeSinceLastPong =
					previousLastPong > 0
						? pongReceivedAt - previousLastPong
						: "unknown";
	...
	private cleanupHandlers(): void {
		...
		try {
			...
			logger.warn("Error cleaning up handlers", {
				connectionId: this.connectionId,
				error: err,
			});
		}
	...
	private handleDisconnection(code?: number, reason?: string): void {
		if (this.hotReloadDetected && this.ws?.readyState === WebSocket.OPEN) {
			logger.info(
				"Hot reload in progress - ignoring disconnection (connexion ouverte preservee)",
				{
					connectionId: this.connectionId,
					code,
					reason,
		...
		if (this.hotReloadDetected && this.ws?.readyState !== WebSocket.OPEN) {
			logger.info(
				"Hot reload in progress mais connexion fermee - traitement de la deconnexion",
				{
					connectionId: this.connectionId,
					code,
					reason,
		...
		if (this.reconnectAttempts >= maxAttempts) {
			...
			if (this.reconnectTimeout) {
				...
				logger.debug(
					"Cleared pending reconnect timeout after max attempts",
					{ connectionId: this.connectionId }
				);
			}
		...
		if (disconnectionInfo.type === "PONG_TIMEOUT") {
			logger.warn(
				"⏱️ PONG_TIMEOUT detected - reconnecting automatically",
				reconnectInfo
			);
		} else {
		...
		switch (code) {
			...
			case 1001:
				return {
					type: "GOING_AWAY",
					description:
						"Going away (1001) - page closing or server restart",
					shouldReconnect: false,
					isServerError: false,
				};
			...
			case 1005:
				return {
					type: "NO_STATUS",
					description:
						"No status received (1005) - possible network error",
					shouldReconnect: true,
					isServerError: true,
				};
			...
			case 1008:
				if (
					reason === "PONG_TIMEOUT" ||
					reason?.includes("PONG_TIMEOUT") ||
					reason?.includes("timeout")
				) {
					return {
						type: "PONG_TIMEOUT",
						description:
							"Pong timeout (1008) - connection lost due to heartbeat timeout",
						shouldReconnect: true,
						isServerError: false,
					};
				}
	...
	private handleConnectionError(): void {
		if (this.hotReloadDetected && this.ws?.readyState === WebSocket.OPEN) {
			logger.info(
				"Hot reload in progress - ignoring connection error (connexion ouverte preservee)",
				{
					connectionId: this.connectionId,
					wsState: this.getWebSocketStateName(this.ws?.readyState),
				}
		...
		if (this.hotReloadDetected && this.ws?.readyState !== WebSocket.OPEN) {
			logger.info(
				"Hot reload in progress mais connexion fermee - traitement de l'erreur",
				{
					connectionId: this.connectionId,
					wsState: this.getWebSocketStateName(this.ws?.readyState),
				}
		...
		if (this.reconnectAttempts >= maxAttempts) {
			...
			logger.error(
				"❌ Max reconnect attempts reached in handleConnectionError - giving up",
				{
					connectionId: this.connectionId,
					attempts: this.reconnectAttempts,
					maxAttempts,
	...
	private scheduleReconnect(isServerUnreachable: boolean = false): void {
		...
		if (this.reconnectAttempts >= maxAttempts) {
			logger.error(
				"Cannot schedule reconnect - max attempts already reached",
				{
					connectionId: this.connectionId,
					attempts: this.reconnectAttempts,
					maxAttempts,
		...
		if (this.reconnectTimeout) {
			logger.debug("Cancelling previous reconnect timeout", {
				connectionId: this.connectionId,
			});
			clearTimeout(this.reconnectTimeout);
			this.reconnectTimeout = null;
		...
		if (this.isConnecting) {
			logger.warn(
				"Connection already in progress, not scheduling reconnect",
				{ connectionId: this.connectionId }
			);
			return;
		}
		...
		this.reconnectTimeout = setTimeout(() => {
			...
			if (this.reconnectAttempts >= maxAttempts) {
				logger.error(
					"Reconnect timeout fired but max attempts already reached, cancelling",
					{
						connectionId: this.connectionId,
						attempts: this.reconnectAttempts,
						maxAttempts,
			...
			if (!this.pendingPlayerName) {
				...
				if (storedName) {
					...
					logger.debug(
						"Using authenticated user for reauthentication",
						{
							connectionId: this.connectionId,
						}
					);
				...
				const checkConnection = setInterval(() => {
					if (this.ws?.readyState === WebSocket.OPEN) {
						...
						logger.info("🔄 Reauthentification apres reconnexion", {
							connectionId: this.connectionId,
							playerName: this.pendingPlayerName?.replace(
								/token=[^&]+/,
								"token=***"
							),
							roomId,
						...
						if (roomId) {
							setTimeout(() => {
								...
								if (store.connection.playerId) {
									logger.info(
										"🔄 Rejoindre la room apres reconnexion",
										{
											connectionId: this.connectionId,
											roomId,
											playerId: store.connection.playerId,
									...
									setTimeout(() => {
										if (
											...
											logger.info(
												"🔄 Demande de l'etat de la room apres reconnexion",
												{
													connectionId:
														this.connectionId,
													roomId,
									...
									logger.warn(
										"PlayerId not available after authentication, retrying join",
										{
											connectionId: this.connectionId,
											roomId,
										}
	...
	authenticate(playerName: string): void {
		...
		if (!safeName) {
			const stored = (
				sessionStorage.getItem("bombparty_player_name") || ""
			).trim();
			if (stored) {
				safeName = stored;
			} else {
			...
			logger.warn("Authentication fallback to safe name", {
	...
	createRoom(
		...
		if (!this.ws) {
			logger.error("Cannot create room: WebSocket is null", undefined, {
				connectionId: this.connectionId,
			});
			store.setLastError(
				"Connexion WebSocket non initialisee. Veuillez rafraîchir la page."
			);
			return;
		}
		...
		if (!store.connection.playerId) {
			...
			store.setLastError(
				"Vous devez être authentifie pour creer un lobby. Veuillez patienter..."
			);

			if (store.connection.isAuthenticating) {
				logger.info(
					"Authentication in progress, will retry createRoom in 2 seconds"
				);
				setTimeout(() => {
					if (useBombPartyStore.getState().connection.playerId) {
						...
						logger.error(
							"Authentication still not complete after wait"
						);
						useBombPartyStore
							.getState()
							.setLastError(
								"Authentification echouee. Veuillez rafraîchir la page."
							);
					}
				}, 2000);
			}
		...
		if (store.connection.state !== "connected") {
			logger.error(
				"Cannot create room: connection state is not connected",
				undefined,
				{
					connectionId: this.connectionId,
					connectionState: store.connection.state,
			...
			store.setLastError(
				`etat de connexion invalide: ${store.connection.state}. Veuillez rafraîchir la page.`
			);
			return;
		}
		...
		try {
			...
			logger.info("Create room message sent successfully", {
				connectionId: this.connectionId,
			});
		} catch (error) {
			logger.error("Error sending create room message", error, {
				connectionId: this.connectionId,
			});
			store.setLastError(
				"Erreur lors de l'envoi de la requête. Veuillez reessayer."
			);
		}
	}
	...
	joinRoom(roomId: string, password?: string): void {
		...
		if (this.ws?.readyState === WebSocket.OPEN) {
			if (!store.connection.playerId) {
				logger.error("Cannot join: not authenticated", undefined, {
					connectionId: this.connectionId,
				});
			...
			logger.error("Cannot join: WebSocket not open", undefined, {
				connectionId: this.connectionId,
			});
	...
	submitWord(word: string, msTaken: number): void {
		...
		if (!store.connection.roomId) {
			logger.error("submitWord - No roomId", {
				connectionId: this.connectionId,
			});
		...
		try {
			...
			logger.error("submitWord - Error sending message", error, {
				connectionId: this.connectionId,
			});
			this.pendingMessages.push(message);
	...
	requestLobbyList(): void {
		if (this.ws?.readyState === WebSocket.OPEN) {
			logger.debug("Request lobby list", {
				connectionId: this.connectionId,
			});
			this.ws.send(
				JSON.stringify({
			...
			logger.debug("Cannot request list: WebSocket not open", {
				connectionId: this.connectionId,
			});
		}
	}
	...
	disconnect(): void {
		if (this.isDisconnecting) {
			logger.debug("Disconnection already in progress", {
				connectionId: this.connectionId,
			});
			return;
		}
		...
		if (this.hotReloadDetected && this.ws?.readyState === WebSocket.OPEN) {
			logger.info(
				"Hot reload detected - skipping disconnection to preserve open connection",
				{
					connectionId: this.connectionId,
					wsState: this.getWebSocketStateName(this.ws?.readyState),
					isConnected: true,
		...
		if (this.hotReloadDetected && this.ws?.readyState !== WebSocket.OPEN) {
			logger.info(
				"Hot reload detected but connection closed - allowing disconnection for cleanup",
				{
					connectionId: this.connectionId,
					wsState: this.getWebSocketStateName(this.ws?.readyState),
					isConnected: false,
