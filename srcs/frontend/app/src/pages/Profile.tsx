...
export default function Profile() {
	...
	// États pour la gestion des amis (de Friends.tsx)
	...
	// Fonctions de gestion des amis (de Friends.tsx)
	...
	const fetchData = async () => {
		...
		setLoading(true);
		try {
			// Récupérer les amis, requests et blocked users
			await Promise.all([
				fetchFriends(),
				fetchRequests(),
				fetchBlockedUsers(),
	...
	// WebSocket pour les amis (de Friends.tsx)
	useEffect(() => {
		if (!token || !user?.id) return;

		const ws = new WebSocket(
			`ws://localhost:3000/ws-friends?token=${token}`
		);
		wsRef.current = ws;
															...
															<div className="flex items-center gap-4">
																...
																<div className="flex gap-2">
																	{isOnline(
																		...
																		<button
																			onClick={() => {
																				// Send invitation via WS without navigating; navigation will happen when game starts
																				if (
																					pongWsRef
																						.current
																						?.readyState ===
