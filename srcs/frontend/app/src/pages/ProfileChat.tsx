...
interface PublicUser {
  id: number;
  display_name: string; // ou "name" selon ta DB, ici on affiche le pseudo
  avatar?: string;
...
export default function PublicProfile() {
  ...
  // On récupère le paramètre "name" depuis l'URL (défini dans la route /profile/:name)
  ...
  useEffect(() => {
    if (name && token) {
      // On décode le nom (ex: "Jean%20Pierre" devient "Jean Pierre")
  ...
  const loadProfileData = async (userName: string) => {
    ...
    setError(null);
    try {
      // Étape 1 : Récupérer les infos de l'utilisateur via son NOM
      const user = await fetchUserByName(userName);
      
      if (user && user.id) {
        setTargetUser(user);
        
        // Étape 2 : Une fois qu'on a l'ID, on récupère les stats et l'historique
        await Promise.all([
          fetchUserStats(user.id),
          fetchMatchHistory(user.id, 1) // Page 1
        ]);
  ...
  const fetchMatchHistory = async (userId: number, page: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setHasMoreHistory(matches.length === 10); // Si on reçoit moins de 10 matchs, c'est fini
      }
    } finally {
