...
export function initializeSyllables(): void {
  if (Array.isArray(syllablesData)) {
    ...
    console.log(`[BombParty Frontend] ${availableSyllables.length} syllabes chargees depuis syllabes.json`);
  } else {
...
export function getRandomSyllable(excludeSyllable?: string): string {
  ...
  if (candidates.length === 0) {
    console.log('[BombParty Frontend] Toutes les syllabes sont recentes, reduction de l\'historique...');
