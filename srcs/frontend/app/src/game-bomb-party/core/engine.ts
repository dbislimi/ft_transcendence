...
import { BONUS_WEIGHTS, MAX_BONUS_PER_TYPE } from "../../types/bombparty";
...
export class BombPartyEngine {
	...
	startGame(config: GameConfig, player1Name?: string): void {
		...
		for (let i = 0; i < config.playersCount; i++) {
			const defaultName =
				i === 0 && player1Name ? player1Name : `Joueur ${i + 1}`;
			players.push({
				id: `player-${i + 1}`,
				name: defaultName,
		...
		this.state = {
			...
			turnDirection: 1,
			baseTurnSeconds: Math.max(
				3,
				Math.floor(config.turnDurationMs / 1000)
			),
			activeTurnEndsAt: undefined,
			history: [],
		};
	...
	async startTurn(): Promise<void> {
		if (this.state.players[this.state.currentPlayerIndex]?.isEliminated) {
			this.nextPlayer();
			if (
				this.state.players[this.state.currentPlayerIndex]?.isEliminated
			) {
	...
	private async getNewSyllable(): Promise<string> {
		try {
			const result = await bombPartyApiService.getRandomSyllable(
				this.lastSyllable
			);
	...
	async resolveTurn(wordValid: boolean, timeExpired: boolean): Promise<void> {
		...
		if (!wordValid || timeExpired) {
			...
			if (currentPlayer.lives === 0) {
				...
				console.log(
					"[BombPartyEngine] Joueur elimine:",
					currentPlayer.name
				);
			}
		}
	...
	private nextPlayer(): void {
		...
		do {
			...
			this.state.currentPlayerIndex =
				(this.state.currentPlayerIndex + step + len) % len;
			attempts++;
			if (attempts > maxAttempts) {
				console.error(
					"Erreur: Impossible de trouver le prochain joueur"
				);
				break;
			}
		} while (
	...
	async getCurrentSyllableInfo(): Promise<{
		syllable: string;
	...
	activateBonus(
		...
		if (!p.bonuses[bonusKey] || p.bonuses[bonusKey] <= 0)
			return { ok: false };

		switch (bonusKey) {
			case "inversion":
				this.state.turnDirection =
					this.state.turnDirection === 1 ? -1 : 1;
				p.bonuses.inversion -= 1;
				return { ok: true };
			case "plus5sec":
				if (
					this.state.phase === "TURN_ACTIVE" &&
					this.state.activeTurnEndsAt
				) {
			...
			case "vitesseEclair":
				...
				const targetId =
					targetIdx >= 0
						? this.state.players[targetIdx].id
						: undefined;
				if (targetId) {
