      ...
      (async () => {
        try {
          if (engine && engine.getWordSuggestions) {
            const newSuggestions = await engine.getWordSuggestions(5);
            const info = await engine.getCurrentSyllableInfo();
            setSuggestions(newSuggestions);
            setSyllableInfo(info);
          } else {
            // If engine not available, fetch directly from API
            const newSuggestions = await bombPartyApiService.getWordSuggestions(syllable, 5);
            setSuggestions(newSuggestions);
            setSyllableInfo(null);
          }
        } catch (error) {
          console.error('[WordInput] Error fetching suggestions:', error);
          setSuggestions([]);
        }
      })();
    }
        ...
        {items.map((it) => {
          ...
          return (
            <button
              ...
              className={`relative w-12 h-12 rounded-xl text-2xl flex items-center justify-center transition-all duration-300 transform ${disabled
            ...
            >
              ...
              <span className={`relative z-10 transform transition-transform duration-300 ${isProcessing ? 'animate-spin-bonus' : ''
                }`}>
                {it.icon}
              </span>

              {count > 0 && (
                <span className={`absolute -top-1 -right-1 text-[10px] font-bold ${rarityStyles.badge} rounded-full w-6 h-6 flex items-center justify-center border-2 border-white/30 shadow-lg transform transition-all duration-300 ${!disabled ? 'hover:scale-125' : ''
        ...
        <div className="bg-slate-800/90 backdrop-blur-md rounded-xl border border-cyan-500/30 p-6 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              <div className={`text-sm font-medium transition-colors duration-300 ${hasDoubleChance
