			...
			{players.map((player, index) => {
				...
				return (
					...
					>
						...
						<div
							className={`relative bg-slate-800/90 backdrop-blur-md rounded-xl border p-3 min-w-[110px] text-center transition-all duration-300 ${
								isCurrent && !isEliminated
						...
						>
							...
							<div className="flex items-center justify-center gap-1 mb-1 text-sm">
								{pendingFastForNextPlayerId === player.id &&
									!isEliminated && (
										<span
											title={t("bombParty.hud.fastTurn")}
											className="text-yellow-300"
										>
											⚡
										</span>
									)}
								{player.pendingEffects?.doubleChance &&
									!isEliminated && (
										<span
											title="Double chance"
							...
							<div className="text-xs px-2 py-1 rounded-full bg-slate-600/50 text-slate-400">
								{isEliminated
									? t("bombParty.hud.eliminated")
									: isCurrent
									? t("bombParty.hud.yourTurn")
									: t("bombParty.hud.waiting")}
							</div>
						</div>
					</div>
				);
