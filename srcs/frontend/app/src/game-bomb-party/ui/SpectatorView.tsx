...
import type { Player, GameState } from "../../types/bombparty";
...
export default function SpectatorView({
	...
	const playerStats = useMemo(() => {
		return gameState.players.map((player) => {
			...
			return {
				...
				averageResponseTime,
				isAlive: !player.isEliminated && player.lives > 0,
			};
		});
	...
	const winner = isGameOver
		? (gameState as any).winner ||
		  gameState.players.find((p) => !p.isEliminated && p.lives > 0)
		: null;
	...
	return (
		<div className="fixed inset-0 bg-slate-900/95 backdrop-blur-sm z-50 flex items-center justify-center p-4">
			<div className="bg-slate-800/90 backdrop-blur-md rounded-xl border border-slate-600 shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
				{isGameOver && (
					<div
						className={`sticky top-0 z-20 p-8 text-center border-b ${
							didPlayerWin
					...
					>
						...
						{winner && (
							<p className="text-lg text-slate-400">
								{t("bombParty.gameOver.livesRemaining", {
									count: winner.lives,
								})}
							</p>
						)}
					...
					<div className="space-y-3">
						{sortedStats.map((stat) => {
							const isCurrentPlayer =
								stat.player.id === currentPlayerId;
							const winRate =
										...
										>
											<div className="flex items-center gap-2">
												<span className="text-slate-300 font-medium">
													{player?.name ||
														t(
															"bombParty.spectator.unknownPlayer"
														)}
												</span>
												<span
													className={`font-mono ${
											...
											<div className="flex items-center gap-3 text-xs text-slate-400">
												<span>
													{entry.ok
														? t(
																"bombParty.spectator.valid"
														  )
														: t(
																"bombParty.spectator.invalid"
														  )}
												</span>
												<span>
													{(
