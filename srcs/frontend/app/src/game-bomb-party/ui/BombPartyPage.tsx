...
export default function BombPartyPage() {
	...
	const handleLobbyCreate = useCallback(
		(meta: { name: string; isPrivate: boolean; password?: string }) => {
			...
			const countdownInterval = setInterval(() => {
				setCountdown((prev) => {
					if (prev <= 1) {
						...
						engine.startTurn();
						timer.startTurn(
							engine.getTurnDurationForCurrentPlayer()
						);
						setGameState(engine.getState());
						...
						setTimeout(() => {
							...
							console.log(
								"⏱️ Délai de grâce terminé - timeout autorisé"
							);
						}, 5000);
	...
	return (
		<BackgroundSurface game="bombparty">
			...
			<div className="min-h-screen relative overflow-hidden">
				...
				<Countdown
					count={countdown}
					isActive={gamePhase === "GAME" && countdown > 0}
				/>
						...
						<div className="bg-slate-800/90 backdrop-blur-md rounded-2xl border border-purple-500/30 p-8 max-w-md text-center">
							...
							{engine.getWinner() && (
								<p className="text-slate-300 text-xl mb-6">
									{t("bombParty.gameOver.winner", {
										name: engine.getWinner()?.name,
									})}
								</p>
							)}
							<button
				...
				<div className="relative w-full h-screen">
					...
					<PlayerCircle
						...
						pendingFastForNextPlayerId={
							gameState.pendingFastForNextPlayerId
						}
					...
					<DraggablePanel initialOffset={{ x: 0, y: -120 }}>
						<WordInput
							...
							hasDoubleChance={Boolean(
								engine.getCurrentPlayer()?.pendingEffects
									?.doubleChance
							)}
						/>
					</DraggablePanel>

					{/* Bouton retour au menu (positionnement via wrapper) */}
					...
					<div className="text-xs text-slate-400 mb-2">
						...
						{trigramInfo && (
							<span className="ml-2 text-[10px] text-slate-500">
								{trigramInfo.availableWords}/
								{trigramInfo.totalWords}
							</span>
						)}
					</div>
...
function DraggablePanel({
	...
	React.useEffect(() => {
		...
		const next = {
			x: Math.round(w / 2) + initialOffset.x,
			y: Math.round(h / 2) + initialOffset.y,
		};
		setPos({
			x: clamp(next.x, 16, w - 320),
			y: clamp(next.y, 16, h - 120),
		});
