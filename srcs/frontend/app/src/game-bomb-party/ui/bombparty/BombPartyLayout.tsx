...
export default function BombPartyLayout({
	...
	const previousPlayerIndexRef = useRef<number>(
		state.gameState.currentPlayerIndex
	);
	const previousBonusNotificationRef =
		useRef<typeof state.bonusNotification>(null);
	const [turnTransitionTrigger, setTurnTransitionTrigger] = useState<
		number | null
	>(null);
	const [bonusFlashTrigger, setBonusFlashTrigger] = useState<{
		timestamp: number;
		icon: string;
		name: string;
	} | null>(null);
	...
	const currentPlayer = state.gameState.players.find(
		(p: Player) => p.id === state.playerId
	);
	const isCurrentPlayerEliminated = currentPlayer?.isEliminated ?? false;

	const activePlayers = state.gameState.players.filter(
		(p: Player) => !p.isEliminated
	);
	const eliminatedPlayers = state.gameState.players.filter(
		(p: Player) => p.isEliminated
	);
	...
	const currentPlayerIndexInActive = activePlayers.findIndex(
		(p: Player) => p.id === currentPlayerInGame?.id && !p.isEliminated
	);
	...
	useEffect(() => {
		if (
			previousPlayerIndexRef.current !==
				state.gameState.currentPlayerIndex &&
	...
	useEffect(() => {
		if (
			...
			state.bonusNotification !== previousBonusNotificationRef.current
		) {
			...
			setBonusFlashTrigger({
				...
				name:
					bonusNames[state.bonusNotification.bonusKey] ||
					state.bonusNotification.bonusKey,
			});
			playSound("bonus");
	...
	if (
		(state.gameState.phase === "GAME_OVER" && gameMode === "multiplayer") ||
		(isCurrentPlayerEliminated && gameMode === "multiplayer")
	) {
		return (
			...
			<div className="min-h-screen relative overflow-hidden">
				...
				<div className="relative w-full h-screen">
					<PlayerCircle
						...
						currentPlayerIndex={
							currentPlayerIndexInActive >= 0
								? currentPlayerIndexInActive
								: 0
						}
						radiusBoost={radiusBoost}
						pendingFastForNextPlayerId={
							state.gameState.pendingFastForNextPlayerId
						}
					...
					{eliminatedPlayers.length > 0 && (
						<div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 bg-slate-800/80 backdrop-blur-md border border-slate-600 rounded-lg p-3 max-w-md">
							<div className="text-xs text-slate-400 mb-2 text-center">
								{t(
									"bombParty.spectator.eliminatedPlayers",
									"Joueurs elimines"
								)}
							</div>
