...
export function useBombPartyHooks(user: any) {
  ...
  useEffect(() => {
    ...
    if (gameMode === 'multiplayer' && storeRoomId && gamePhase === 'RULES') {
      logger.debug('empêcher reset gamePhase à rules pendant le jeu', { roomId: storeRoomId });
      setGamePhase('GAME');
    }
  ...
  const handleLobbyCreate = useCallback((meta: { name: string; isPrivate: boolean; password?: string; maxPlayers: number; }) => {
    if (gameMode !== 'multiplayer') {
      logger.debug('forcer mode multijoueur lors de la creation du lobby');
  ...
  const startGame = useCallback((config: GameConfig) => {
    logger.debug('startgame appeler', { config, gameMode });
    ...
    if (gameMode === 'local') {
      ...
      if (!wsHook.roomId) {
        logger.warn('pas de roomid pour démarrer le jeu');
        return;
      }
      logger.debug('envoi bp:lobby:start au serveur', { roomId: wsHook.roomId });
  ...
  const handleBackToMenu = useCallback(() => {
    ...
    if (mode === 'local') {
      ...
      logger.debug('countdown démarré en mode local', { countdown: 3, phase: 'GAME' });
      
      let count = 3;
      countdownIntervalRef.current = timerService.setInterval(() => {
        count--;
        setCountdown(count);
        logger.debug('countdown local', { count });
        ...
        if (count === 0) {
          ...
          logger.debug('jeu démarré en mode local', {
            turnStartedAt: engineState.turnStartedAt,
            turnEndsAt: engineState.turnEndsAt,
            turnDurationMs: engineState.turnDurationMs,
            turnStartTime: turnStart,
            timerStarted: !!(gameHook.timer && turnStart && turnDuration)
          });
        }
      }, 1000);
    } else {
      logger.debug('mode multijoueur sélectionné', { multiplayerType: multiplayerTypeParam });
