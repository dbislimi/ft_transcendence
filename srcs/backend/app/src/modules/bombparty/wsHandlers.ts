...
const bombPartyWSHandlers: FastifyPluginAsync = async (fastify) => {
	...
	fastify.get(
		...
		(connection: any, request) => {
			const { socket } = connection as { socket: any };
			...
			async function authenticatePlayer(
				...
				if (userAuthenticated && typeof userId === "number") {
					try {
						const user = await new Promise<any>(
							(resolve, reject) => {
								fastify.db.get(
						...
						if (user) {
							const dbName = user.display_name || user.name;
							if (dbName) {
								finalPlayerName = sanitizePlayerName(dbName);
								bombPartyLogger.info(
									{ userId, displayName: finalPlayerName },
						...
						bombPartyLogger.warn(
							{ userId, error },
							"Failed to fetch user display name from DB, using provided name"
						);
					}
				}
			...
			socket.on("message", async (data: Buffer) => {
				try {
					...
					if (rawMessage.event === "bp:auth") {
						...
						const authPayload = validation.data!.payload as any;
						if (await authenticatePlayer(authPayload.playerName)) {
							sendMessage({
					...
					switch (message.event) {
						...
						case "bp:lobby:join":
							await handleLobbyJoin(playerId, message.payload);
							break;

						case "bp:lobby:leave":
							await handleLobbyLeave(playerId, message.payload);
							break;

						case "bp:lobby:start":
							await handleLobbyStart(playerId, message.payload);
							break;
						...
						case "bp:game:input":
							await handleGameInput(playerId, message.payload);
							break;

						case "bp:bonus:activate":
							await handleBonusActivate(
								playerId,
								message.payload
							);
							break;
				...
				if (session.playerId && session.roomId) {
					...
					setTimeout(() => {
						...
						const hasActiveSocket =
							...
							Array.from(roomPlayer.sockets).some(
								(ws) => ws.readyState === 1
							); // OPEN = 1
						...
						void roomManager.leaveRoom(playerId, roomId, socket);
					}, gracePeriodMs);
				}
			});
