...
export class DictionaryManager {
	...
	async initialize(): Promise<void> {
		...
		if (
			!fs.existsSync(this.partitionsDir) ||
			this.getPartitionFiles().length === 0
		) {
	...
	private getPartitionFiles(): string[] {
		...
		return fs
			.readdirSync(this.partitionsDir)
			.filter(
				(file) => file.startsWith("partition_") && file.endsWith(".txt")
			)
			.sort((a, b) => {
	...
	private async createPartitions(): Promise<void> {
		...
		this.partitionFiles = this.getPartitionFiles();
		console.log(
			`[DictionaryManager] ${this.partitionCount} partitions creees`
		);
	}
