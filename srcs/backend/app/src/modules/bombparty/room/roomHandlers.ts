...
export function handleJoinRoom(
	...
	if ((room as any).emptyRoomTimeout) {
		...
		bombPartyLogger.info(
			{ roomId, playerId },
			"✅ Joueur rejoint - annulation suppression room vide"
		);
	}
	const playersList = getPlayersList(room!);
	broadcastToRoom(
...
export function handleLeaveRoom(
	...
	if (room.hostId === playerId && room.players.size > 1) {
		const remainingPlayers = Array.from(room.players.keys()).filter(
			(id) => id !== playerId
		);
	...
	if (roomPlayer) {
		if (ws && roomPlayer.sockets && roomPlayer.sockets.size > 1) {
			roomPlayer.sockets.delete(ws as any);
			if (roomPlayer.ws === (ws as any) && roomPlayer.sockets.size > 0) {
...
export function handleStartGame(
	...
	if (!room) {
		bombPartyLogger.warn(
			{ roomId, playerId },
			"Room not found in handleStartGame"
		);
		return { success: false, error: "Room not found" };
	}
	...
	if (!isHost(room, playerId)) {
		bombPartyLogger.warn(
			{ roomId, playerId, hostId: room.hostId },
...
export async function handleGameInput(
	...
	try {
		...
		if (!engine) {
			bombPartyLogger.warn(
				{ roomId, playerId },
				"Game not found in handleGameInput"
			);
		...
		if (!isCurrentPlayer(state, playerId)) {
			bombPartyLogger.warn(
				{ roomId, playerId, currentPlayerId: state.currentPlayerId },
				"Not player turn in handleGameInput"
			);
		...
		bombPartyLogger.error(
			{ roomId, playerId, error },
			"Error in handleGameInput"
		);
		return {
			success: false,
			error: "Internal server error processing game input",
		};
	}
}
...
export function handleActivateBonus(
	...
	try {
		...
		if (!engine) {
			bombPartyLogger.warn(
				{ roomId, playerId, bonusKey },
				"Game not found in handleActivateBonus"
			);
			return { success: false, error: "Game not found" };
		}
		...
		if (!bonusCheck.allowed) {
			bombPartyLogger.warn(
				{ roomId, playerId, bonusKey, reason: bonusCheck.reason },
				"Cannot activate bonus"
			);
			return {
				success: false,
				error: bonusCheck.reason || "Cannot activate bonus",
			};
		}
		...
		if (result.ok) {
			...
			if (!room) {
				bombPartyLogger.warn(
					{ roomId, playerId },
					"Room not found when broadcasting bonus"
				);
				return { success: false, error: "Room not found" };
			}
			broadcastToRoom(room, {
		...
		bombPartyLogger.error(
			{ roomId, playerId, bonusKey, error },
			"Error in handleActivateBonus"
		);
		return {
			success: false,
			error: "Internal server error activating bonus",
		};
	}
}
	...
	if (player.roomId) {
		...
		if (room) {
			const wasCurrentPlayer =
				engine?.getCurrentPlayer()?.id === playerId;
			room.players.delete(playerId);
...
function calculateStateDelta(
	...
	const playerIndexChanged =
		prevState.currentPlayerIndex !== currentState.currentPlayerIndex;
		...
		for (let i = 0; i < currentState.players.length; i++) {
			...
			if (
				...
				const bonusesChanged =
					prev.bonuses.inversion !== curr.bonuses.inversion ||
	...
	if (prevState.usedWords.length !== currentState.usedWords.length) {
		...
		delta.newWords = currentState.usedWords.slice(
			prevState.usedWords.length
		);
		changeCount++;
	}
	...
	if (currentState.phase === "GAME_OVER") {
		const currentWinner =
			winner !== undefined ? winner : currentState.winner;
