...
import { BombPartyEngine } from '../../modules/bombparty/GameEngine.js';
import { giveRandomBonus } from '../../modules/bombparty/engine/engineRules.js';
import type { GameState } from '../../modules/bombparty/types.js';
import { MAX_BONUS_PER_TYPE } from '../../modules/bombparty/shared-types.js';
...
describe("BombPartyEngine", () => {
	...
	describe("resolveTurn", () => {
		...
		it("should reduce lives when word is invalid", () => {
			const initialState = engine.getState();
			const initialLives =
				initialState.players[initialState.currentPlayerIndex].lives;

			engine.resolveTurn(false, false);

			const newState = engine.getState();
			const currentPlayer =
				newState.players[initialState.currentPlayerIndex];
	...
	describe("giveRandomBonus - weighted distribution", () => {
		...
		it("should respect the maximum bonus limit per type", () => {
			...
			expect(testState.players[0].bonuses.extraLife).toBe(
				initialBonuses.extraLife
			);
			expect(testState.players[0].bonuses.inversion).toBe(
				initialBonuses.inversion
			);
			expect(testState.players[0].bonuses.plus5sec).toBe(
				initialBonuses.plus5sec
			);
			expect(testState.players[0].bonuses.doubleChance).toBe(
				initialBonuses.doubleChance
			);
			expect(testState.players[0].bonuses.vitesseEclair).toBe(
				initialBonuses.vitesseEclair
			);
		});
		...
		it("should not exceed maximum bonus limit when adding new bonus", () => {
			...
			expect(testState.players[0].bonuses.extraLife).toBeLessThanOrEqual(
				MAX_BONUS_PER_TYPE
			);
		});
		...
		it("should distribute bonuses according to weights (statistical test)", () => {
			...
			for (let i = 0; i < iterations; i++) {
				...
				else if (player.bonuses.vitesseEclair > 0)
					distribution.vitesseEclair++;
				else if (player.bonuses.plus5sec > 0) distribution.plus5sec++;
				else if (player.bonuses.doubleChance > 0)
					distribution.doubleChance++;
				else if (player.bonuses.inversion > 0) distribution.inversion++;
			}

			const extraLifePercent =
				(distribution.extraLife / iterations) * 100;
			expect(extraLifePercent).toBeGreaterThan(3);
			expect(extraLifePercent).toBeLessThan(8);

			const inversionPercent =
				(distribution.inversion / iterations) * 100;
			...
			const doubleChancePercent =
				(distribution.doubleChance / iterations) * 100;
			expect(doubleChancePercent).toBeGreaterThan(20);
			expect(doubleChancePercent).toBeLessThan(30);

			const vitesseEclairPercent =
				(distribution.vitesseEclair / iterations) * 100;
		...
		it("should only select from available bonuses when some are at max", () => {
			...
			expect(testState.players[0].bonuses.plus5sec).toBeLessThanOrEqual(
				MAX_BONUS_PER_TYPE
			);
			expect(
				testState.players[0].bonuses.doubleChance
			).toBeLessThanOrEqual(MAX_BONUS_PER_TYPE);
			expect(
				testState.players[0].bonuses.vitesseEclair
			).toBeLessThanOrEqual(MAX_BONUS_PER_TYPE);
		});
	});
});
