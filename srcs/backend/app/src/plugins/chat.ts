...
import { verifyTokenFromQuery } from "../utils/auth.ts";
...
export default fp(async function Chat(fastify: FastifyInstance) {
  ...
  fastify.get("/chat", { websocket: true }, (socket, req) => {
    ...
    if (!token) {
      fastify.log.error("Token manquant dans les paramètres de requête");
      socket.close();
      return;
    }
    ...
    if (!decoded) {
      fastify.log.error("Échec de l'authentification WebSocket - token invalide");
      socket.close();
      return;
    }
    ...
    clients.push(client);
    fastify.log.info(`${client.name} connecté au chat`);
