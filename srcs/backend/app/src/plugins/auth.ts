...
export default fp(async function authPlugin(fastify: FastifyInstance) {
	...
	fastify.post("/check-user", async (request, reply) => {
		...
		try {
			...
			if (existingEmail) {
				return reply
					.code(409)
					.send({ exists: true, error: "Email dejà utilise." });
			}
			...
			if (existingDisplayName) {
				return reply.code(409).send({
					exists: true,
					error: "Ce pseudo est dejà utilise.",
				});
			}
	...
	fastify.post("/register", async (request, reply) => {
		...
		if (!nameRegex.test(name)) {
			return reply.code(400).send({
				error: "Le nom doit commencer par une majuscule suivie uniquement de lettres minuscules.",
			});
		}
		...
		try {
			...
			if (existingDisplayName) {
				return reply
					.code(409)
					.send({ error: "ce pseudo est deja utilise." });
			}
			...
			try {
				...
				console.error(
					"Error in /register (failed to log details):",
					logErr
				);
			}
	...
	fastify.get("/users/:name", async (request, reply) => {
		...
		try {
			const user = await new Promise<any>((resolve, reject) => {
				db.get(
					"SELECT * FROM users WHERE display_name = ? OR name = ?",
					[name, name],
					(err, row) => {
						if (err) reject(err);
						else resolve(row);
