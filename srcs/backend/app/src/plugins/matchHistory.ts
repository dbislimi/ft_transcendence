...
async function matchHistoryPlugin(fastify: any, opts: any) {
  ...
  fastify.decorate("incrementTournamentsWon", async function(userId: number) {
    return new Promise<void>((resolve, reject) => {
      fastify.db.run(
        ...
        (err: any, rows: Match[]) => {
          if (err) {
            ...
            const matches = rows.map(match => {
              fastify.log.info(`Processing match ${match.id}: is_bot=${match.is_bot}, player1_id=${match.player1_id}, player2_id=${match.player2_id}, bot_difficulty=${match.bot_difficulty}, match_type=${match.match_type}`);
