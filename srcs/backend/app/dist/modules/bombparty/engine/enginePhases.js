import { getAlivePlayers } from './engineState';
export function startCountdown(state) {
    state.phase = 'COUNTDOWN';
}
export function startTurn(state, getNewTrigram, getTurnDuration) {
...
export function resolveTurn(state, wordValid, timeExpired, nextPlayerFn, startTurnFn) {
...
export function isTurnExpired(state) {
    if (state.phase !== 'TURN_ACTIVE')
        return false;
    return Date.now() >= state.turnStartedAt + state.turnDurationMs;
}
export function checkAndEndExpiredTurn(state, resolveTurnFn) {
