import { computeUpdatedUserStats, computeNewUserStats, computeUpdatedTrigramStats, computeNewTrigramStats } from './statsCompute';
export class StatsPersistence {
    ...
    async updateUserStats(userId, matchData) {
        return new Promise((resolve) => {
            this.model.getDatabase().get('SELECT * FROM bp_user_stats WHERE user_id = ?', [userId], (err, existingStats) => {
                if (err) {
                ...
                if (existingStats) {
                    ...
                    this.model.getDatabase().run(`UPDATE bp_user_stats SET 
                total_matches = ?, 
                total_wins = ?, 
               ...
               WHERE user_id = ?`, [
                    ...
                    this.model.getDatabase().run(`INSERT INTO bp_user_stats (
    ...
    async addMatchHistory(userId, matchId, matchData) {
        return new Promise((resolve) => {
            this.model.getDatabase().run(`INSERT INTO bp_match_history (
    ...
    async updateTrigramStats(userId, trigram, isSuccess, responseTime) {
        return new Promise((resolve) => {
            this.model.getDatabase().get('SELECT * FROM bp_trigram_stats WHERE user_id = ? AND trigram = ?', [userId, trigram], (err, existingStats) => {
                if (err) {
                ...
                if (existingStats) {
                    ...
                    this.model.getDatabase().run(`UPDATE bp_trigram_stats SET 
                times_used = ?, 
                success_rate = ?, 
                average_time = ?,
                last_used = CURRENT_TIMESTAMP
               WHERE user_id = ? AND trigram = ?`, [computed.newTimesUsed, computed.newSuccessRate, computed.newAverageTime, userId, trigram], function (err) {
                        if (err) {
                    ...
                    this.model.getDatabase().run(`INSERT INTO bp_trigram_stats (
                user_id, trigram, times_used, success_rate, average_time
              ) VALUES (?, ?, ?, ?, ?)`, [userId, trigram, computed.timesUsed, computed.successRate, computed.averageTime], function (err) {
                        if (err) {
    ...
    async getUserStats(userId) {
        return new Promise((resolve) => {
            this.model.getDatabase().get('SELECT * FROM bp_user_stats WHERE user_id = ?', [userId], (err, row) => {
                if (err) {
    ...
    async getUserMatchHistory(userId, limit = 20, offset = 0) {
        return new Promise((resolve) => {
            this.model.getDatabase().all(`SELECT * FROM bp_match_history 
         WHERE user_id = ? 
         ...
         LIMIT ? OFFSET ?`, [userId, limit, offset], (err, rows) => {
                ...
                const history = rows.map(row => this.model.mapRowToMatchHistory(row));
    ...
    async getUserTrigramStats(userId, limit = 10) {
        return new Promise((resolve) => {
            this.model.getDatabase().all(`SELECT * FROM bp_trigram_stats 
         WHERE user_id = ? 
         ...
         LIMIT ?`, [userId, limit], (err, rows) => {
                ...
                const stats = rows.map(row => this.model.mapRowToTrigramStats(row));
    ...
    async getGlobalRanking(limit = 50) {
        return new Promise((resolve) => {
            this.model.getDatabase().all(`SELECT 
          s.user_id,
          u.name as user_name,
          s.total_wins,
          s.total_matches,
