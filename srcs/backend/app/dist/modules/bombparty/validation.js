 ...
 */
function validateBonusKey(value, fieldName) {
    const validKeys = ['inversion', 'plus5sec', 'vitesseEclair', 'doubleChance', 'extraLife'];
...
export function validateGameInputMessage(payload) {
    ...
    if (!msTakenResult.success)
        return msTakenResult;
    return {
        success: true,
        data: {
            roomId: roomIdResult.data,
...
export function validateBonusActivateMessage(payload) {
    ...
    if (!bonusKeyResult.success)
        return bonusKeyResult;
    return {
        success: true,
        data: {
            roomId: roomIdResult.data,
