...
export default class BotController {
    ...
    scores = [0, 0];
    constructor(options = {}) {
        const { learning_rate = 0.1, discount_factor = 0.9, epsilon = 1, epsilon_decay = 0.00001, epsilon_min = 0.01, training = false, } = options;
        this.start = Date.now();
    ...
    save(episode) {
        fs.writeFileSync(`../AI/qtable_saves/${this.type}/qtable_${this.type}_episode_${episode}.json`, JSON.stringify(this.qTable, null, 2), "utf-8");
    }
    load() {
        try {
            const raw = fs.readFileSync(`../AI/qtable_saves/${this.type}/qtable_${this.type}_episode_${this.qtable_nb}.json`, "utf-8");
