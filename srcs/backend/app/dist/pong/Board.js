...
import { MediumBot } from "./Controller.ts";
...
export default class Board {
    ...
    normHitpoint = 0;
    onWin;
    constructor(onWin, maxScore = 5, height = 100, width = 200) {
        this.onWin = onWin;
    ...
    setBallPos(x = this.width / 2, y = this.height / 2) {
        this.ball.x = x;
        this.ball.y = y;
    }
    bounceBallX(reset) {
    ...
    checkBonusCollision() {
        const player = this.ball.dx > 0 ? 0 : 1;
        if (this.ball.x >= this.width / 2 - this.bonusRadius &&
    ...
    updateBonus(dt) {
        if (this.bonus.length < this.bonusNb) {
            if (this.elapsedTime - this.lastBonusSpawn >=
                this.bonusSpawnInterval) {
                let retries = 2;
                let y = Math.floor(Math.random() * (this.height - this.bonusRadius * 2) +
                    this.bonusRadius);
                ...
                if (retries) {
                    // Générer différents types de bonus aléatoirement
                    const bonusTypes = [Bigger, Smaller, Faster];
                    const randomBonus = bonusTypes[Math.floor(Math.random() * bonusTypes.length)];
                    this.bonus.push(new randomBonus(y, this.bonusRadius));
    ...
    connectBot(id, diff, training = false) {
